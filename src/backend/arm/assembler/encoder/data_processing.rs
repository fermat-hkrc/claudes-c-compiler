...
mod encode_orn_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs:1-7 "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:235 "orn" => encode_orn; ARM ARM Logical (shifted register) ORN opc=01 N=1;
    //   ARM ARM Advanced SIMD three-same ORN 0 Q 0 01110 11 1 Rm 000111 Rn Rd;
    //   GNU as / llvm-mc alias orn Rd, Rn, #imm = orr Rd, Rn, #~imm;
    //   data_processing.rs:753 MVN Rd, Rm -> ORN Rd, XZR, Rm
    // Stronger considered:
    //   - State machine: rejected — encode_orn is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree ORN decoder
    //   - Same-job sibling encode_eon: rejected — EON / opc=10 (different job)
    //   - Same-job sibling encode_logical(opc=01): rejected — ORR / N=0 (different job); used only as N-bit metamorphic
    // Weaker available: algebraic.metamorphic (N XOR vs ORR, MVN alias), algebraic.invariant (word layout),
    //   negative_error (arity / mixed width / SP / FP / shift range / extra operand / NEON T)
    // Differential: candidate=encode_orn, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=operands <-> asm text `orn Rd, Rn, Rm{, shift}` / `orn Rd, Rn, #imm` / `orn Vd.T, Vn.T, Vm.T`

    use super::{encode_logical, encode_mvn, encode_orn};
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_orn_kat_llvm_mc_orn_x0_x1_x2() {
        let want = 0xaa220020u32;
        let mc = llvm_mc_word("orn x0, x1, x2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("x1".into()),
    ...
    #[test]
    fn encode_orn_kat_llvm_mc_orn_w0_w1_w2() {
        let want = 0x2a220020u32;
        let mc = llvm_mc_word("orn w0, w1, w2").expect("llvm-mc W KAT");
        assert_eq!(mc, want, "llvm-mc W KAT mapping broken");
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w1".into()),
    ...
    #[test]
    fn encode_orn_kat_llvm_mc_orn_neon_8b() {
        let want = 0x0ee21c20u32;
        let mc = llvm_mc_word("orn v0.8b, v1.8b, v2.8b").expect("llvm-mc neon 8b KAT");
        assert_eq!(mc, want, "llvm-mc neon 8b KAT mapping broken");
        let ops = [
            Operand::RegArrangement {
                reg: "v0".into(),
                arrangement: "8b".into(),
        ...
        let sut = sut_word(&ops).expect("SUT neon 8b KAT");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_orn_kat_llvm_mc_orn_neon_16b() {
        let want = 0x4ee21c20u32;
        let mc = llvm_mc_word("orn v0.16b, v1.16b, v2.16b").expect("llvm-mc neon 16b KAT");
        assert_eq!(mc, want, "llvm-mc neon 16b KAT mapping broken");
        let ops = [
            Operand::RegArrangement {
                reg: "v0".into(),
                arrangement: "16b".into(),
        ...
        let sut = sut_word(&ops).expect("SUT neon 16b KAT");
        assert_eq!(sut, want);
    }
    ...
    proptest! {
        ...
        #[test]
        fn encode_orn_diff_reg_llvm_mc(
            rd in reg_num(),
            rn in reg_num(),
            rm in reg_num(),
            is_64 in any::<bool>(),
            ...
            let mut asm = format!("orn {}, {}, {}", rd_n, rn_n, rm_n);
            if use_shift {
                ops.push(Operand::Shift {
                    kind: kind.to_string(),
                    amount: amt,
        ...
        #[test]
        fn encode_orn_diff_neon_llvm_mc(
            d in reg_num(),
            n in reg_num(),
            m in reg_num(),
            q16 in any::<bool>(),
            ...
            let arr = if q16 { "16b" } else { "8b" };
            let asm = format!("orn v{}.{}, v{}.{}, v{}.{}", d, arr, n, arr, m, arr);
            let ops = [
                Operand::RegArrangement {
                    reg: format!("v{}", d),
                    arrangement: arr.into(),
        ...
        #[test]
        fn encode_orn_diff_imm_llvm_mc(
            rd in 0u32..=30,
            rn in reg_num(),
            is_64 in any::<bool>(),
            seed in 0u32..10000,
            ...
            let rot = (seed / 6 / (size - 1).max(1)) % size;
            let m = bitmask_from_fields(size, ones, rot, is_64);
            let orn_u = if is_64 { !m } else { (!(m as u32)) as u64 };
            let imm = orn_u as i64;
            let rd_n = gpr(is_64, rd);
            let rn_n = gpr(is_64, rn);
            let hex = if is_64 {
                format!("#0x{:x}", orn_u)
            } else {
                format!("#0x{:x}", orn_u as u32)
            };
            let asm = format!("orn {}, {}, {}", rd_n, rn_n, hex);
            let ops = [
                Operand::Reg(rd_n),
                Operand::Reg(rn_n),
                Operand::Imm(imm),
        ...
        #[test]
        fn encode_orn_metamorphic_n_bit_vs_orr(
            rd in reg_num(),
            rn in reg_num(),
            rm in reg_num(),
            is_64 in any::<bool>(),
            ...
            let orn_w = match encode_orn(&ops) {
                Ok(EncodeResult::Word(w)) => w,
                other => panic!("ORN should encode, got {:?}", other),
            };
            let orr_w = match encode_logical(&ops, 0b01) {
                Ok(EncodeResult::Word(w)) => w,
                other => panic!("ORR should encode, got {:?}", other),
            };
            prop_assert_eq!(
                orn_w ^ orr_w,
                1u32 << 21,
                "ORN N must be 1 vs ORR N=0 (orn={:#010x} orr={:#010x})",
                orn_w,
                orr_w
            );
        }
        ...
        #[test]
        fn encode_orn_metamorphic_mvn_alias(
            rd in reg_num(),
            rm in reg_num(),
            is_64 in any::<bool>(),
            kind in shift_kind(),
            ...
            let mut asm = format!("orn {}, {}, {}", rd_n, gpr(is_64, 31), rm_n);
            if use_shift {
                let sh = Operand::Shift {
                    kind: kind.to_string(),
                    amount: amt,
                };
                orn_ops.push(sh.clone());
                mvn_ops.push(sh);
                if !(kind == "lsl" && amt == 0) {
                    asm.push_str(&format!(", {} #{}", kind, amt));
                }
            }
            let orn_w = sut_word(&orn_ops).expect("ORN ZR form must encode");
            let mvn_w = match encode_mvn(&mvn_ops) {
                Ok(EncodeResult::Word(w)) => w,
                other => panic!("MVN should encode, got {:?}", other),
            };
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid {}: {}", asm, e));
            prop_assert_eq!(orn_w, mvn_w, "ORN Rd, ZR, Rm must equal MVN Rd, Rm for {}", asm);
            prop_assert_eq!(orn_w, mc, "ORN Rd, ZR, Rm must match llvm-mc for {}", asm);
        }

        #[test]
        fn encode_orn_invariant_arm_fields(
            rd in reg_num(),
            rn in reg_num(),
            rm in reg_num(),
            is_64 in any::<bool>(),
            ...
            let w = sut_word(&ops).expect("valid ORN must encode");
            let sf = if is_64 { 1u32 } else { 0 };
            prop_assert_eq!((w >> 31) & 1, sf, "sf");
            prop_assert_eq!((w >> 29) & 3, 0b01, "opc");
            prop_assert_eq!((w >> 24) & 0x1f, 0b01010, "opcode 01010");
        ...
        #[test]
        fn encode_orn_neg_arity(n in 0usize..=2, is_64 in any::<bool>(), r in 0u32..=30) {
            let ops: Vec<Operand> = (0..n)
                .map(|_| Operand::Reg(gpr(is_64, r)))
                .collect();
            prop_assert!(
                encode_orn(&ops).is_err(),
                "fewer than 3 operands must Err, n={}",
                n
            );
        }
        ...
        #[test]
        fn encode_orn_neg_invalid_name(
            is_64 in any::<bool>(),
            rd in 0u32..=30,
            rn in 0u32..=30,
            bad in prop_oneof![
        ...
        #[test]
        fn encode_orn_neg_extra_operand(
            is_64 in any::<bool>(),
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
        ...
        fn encode_orn_neg_mixed_width(
            ...
            prop_assert!(
                encode_orn(&ops).is_err(),
                "mixed-width ORN registers must Err (rd64={} rn64={} rm64={})",
                rd64,
                rn64,
                rm64
            );
        ...
        fn encode_orn_neg_shift_range(
            ...
            prop_assert!(
                encode_orn(&ops).is_err(),
                "out-of-range {} #{} on {}-bit ORN must Err",
                kind,
                amt,
                if is_64 { 64 } else { 32 }
            );
        ...
        #[test]
        fn encode_orn_neg_unknown_shift(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            is_64 in any::<bool>(),
        ...
        #[test]
        fn encode_orn_neg_invalid_neon_arr(
            d in 0u32..=31,
            n in 0u32..=31,
            m in 0u32..=31,
            arr in prop::sample::select(vec!["8h", "4h", "4s", "2s", "2d", "1d"]),
        ) {
            let ops = [
                Operand::RegArrangement {
                    reg: format!("v{}", d),
            ...
            prop_assert!(
                encode_orn(&ops).is_err(),
                "NEON ORN T={} is not 8b/16b and must Err",
                arr
            );
        }
    }

    // Coverage-sweep: ARM ARM sf / Q field metamorphics and remaining documented error paths
    // (mismatched NEON T, bare Operand::Reg in vector form, trailing operand after shift, non-Reg GPR kinds).
    proptest! {
        #![proptest_config(cfg())]

        #[test]
        fn encode_orn_metamorphic_sf(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            kind in shift_kind(),
            ...
            let wx = sut_word(&ops_x).expect("64-bit ORN must encode");
            let ww = sut_word(&ops_w).expect("32-bit ORN must encode");
            prop_assert_eq!(wx ^ ww, 1u32 << 31, "sf must be the only X/W difference");
        }

        #[test]
        fn encode_orn_metamorphic_neon_q(
            d in reg_num(),
            n in reg_num(),
            m in reg_num(),
        ) {
            ...
            let w8 = sut_word(&ops_8).expect("8b ORN must encode");
            let w16 = sut_word(&ops_16).expect("16b ORN must encode");
            prop_assert_eq!(w8 ^ w16, 1u32 << 30, "Q must be the only 8b/16b difference");
        }

        #[test]
        fn encode_orn_neg_neon_mismatch_or_bare(
            d in 0u32..=31,
            n in 0u32..=31,
            m in 0u32..=31,
            which in 0u32..=2,
            ...
            let ops = match which {
                ...
                _ => vec![
                    ...
                    Operand::Reg(format!("x{}", m % 31)),
                ],
            };
            prop_assert!(
                encode_orn(&ops).is_err(),
                "mismatched T or bare Reg in NEON ORN must Err (which={})",
                which
            );
        }

        #[test]
        fn encode_orn_neg_trailing_after_shift(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            is_64 in any::<bool>(),
            ...
            prop_assert!(
                encode_orn(&ops).is_err(),
                "trailing operand after shift must Err (extra={})",
                extra
            );
        }

        #[test]
        fn encode_orn_neg_non_reg_kinds(
            which in 0u32..=2,
            kind in 0u32..=3,
        ) {
            let bad = match kind {
            ...
            prop_assert!(
                encode_orn(&ops).is_err(),
                "non-register kind at GPR operand {} must Err",
                which
            );
        }
    }
    ...
    #[test]
    fn test_encode_orn_regression_imm() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Imm(0xaaaaaaaau32 as i64),
        ...
        match encode_orn(&ops) {
            Ok(EncodeResult::Word(w)) => {
                assert_eq!(
                    w, 0x3200f000u32,
                    "ORN w0, w0, #0xaaaaaaaa must encode as ORR w0, w0, #0x55555555"
                );
            }
            other => panic!(
                "ORN w0, w0, #0xaaaaaaaa must encode as ORR #~imm (llvm-mc 0x3200f000), got {:?}",
                other
            ),
        }
    }
    ...
    #[test]
    fn test_encode_orn_regression_extra_operand() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("x1".into()),
            Operand::Reg("x2".into()),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN x0, x1, x2, x3 must Err; a 4th non-shift operand is not valid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_orn_regression_mixed_width() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("x0".into()),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "mixed-width ORN w0, w0, x0 must Err (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_orn_regression_sp() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN wsp, w0, w0 must Err; register 31 is WZR not WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_orn_regression_fp_reg() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("x1".into()),
            Operand::Reg("x2".into()),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN d0, x1, x2 must Err; FP/SIMD registers are not ORN GPRs (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_orn_regression_shift32() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN w0, w0, w0, lsl #32 must Err; 32-bit shift amount range is [0, 31]"
        );
    }

    #[test]
    fn test_encode_orn_regression_unknown_shift_kind() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN w0, w0, w0, lslx #0 must Err; only lsl/lsr/asr/ror are valid (unknown kinds default to lsl)"
        );
    }

    #[test]
    fn test_encode_orn_regression_invalid_neon_arr() {
        let ops = [
            Operand::RegArrangement {
                reg: "v0".into(),
                arrangement: "8h".into(),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN v0.8h, v0.8h, v0.8h must Err; NEON ORN T is only 8b/16b (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_orn_regression_neon_mismatch() {
        let ops = [
            Operand::RegArrangement {
                reg: "v0".into(),
                arrangement: "8b".into(),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN v0.8b, v0.16b, v0.8b must Err; Vd/Vn/Vm arrangements must match (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_orn_regression_trailing_after_shift() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_orn(&ops).is_err(),
            "ORN w0, w0, w0, lsl #1, x0 must Err; trailing operand after shift is not valid (llvm-mc rejects it)"
        );
    }
}
...
mod encode_sbc_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:283-284 sbc/sbcs dispatch; ARM ARM SBC register form;
    //   codegen/i128_ops.rs:73 emits `sbc x1, x3, x5`
    // Stronger considered:
    //   - State machine: rejected — encode_sbc is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree SBC decoder
    // Weaker available: algebraic.metamorphic (S bit, op vs ADC, NGC alias), algebraic.invariant (ARM fields),
    //   negative_error (arity / non-register / extra shift / mixed width / SP / FP)
    // Differential: candidate=encode_sbc, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=(operands,set_flags)<->asm text `sbc`/`sbcs` Rd, Rn, Rm
    //   NGC alias: encode_sbc(Rd, ZR, Rm) <-> llvm-mc `ngc`/`ngcs` Rd, Rm

    use super::{encode_adc, encode_sbc};
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    fn ngc_mnemonic(set_flags: bool) -> &'static str {
        if set_flags {
            "ngcs"
        } else {
            "ngc"
    ...
    fn sut_word(ops: &[Operand], set_flags: bool) -> Result<u32, String> {
        match encode_sbc(ops, set_flags)? {
            EncodeResult::Word(w) => Ok(w),
            other => Err(format!("expected Word, got {:?}", other)),
        }
    }

    fn adc_word(ops: &[Operand], set_flags: bool) -> Result<u32, String> {
        match encode_adc(ops, set_flags)? {
            EncodeResult::Word(w) => Ok(w),
            other => Err(format!("expected Word, got {:?}", other)),
        }
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_sbc_kat_llvm_mc_sbc_x0_x1_x2() {
        let want = 0xda020020u32;
        let mc = llvm_mc_word("sbc x0, x1, x2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("x1".into()),
    ...
    #[test]
    fn encode_sbc_kat_llvm_mc_sbcs_w0_w1_w2() {
        let want = 0x7a020020u32;
        let mc = llvm_mc_word("sbcs w0, w1, w2").expect("llvm-mc KAT sbcs");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken for sbcs");
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w1".into()),
            Operand::Reg("w2".into()),
        ];
        let sut = sut_word(&ops, true).expect("SUT KAT sbcs");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_sbc_kat_llvm_mc_ngc_x0_x1() {
        let want = 0xda0103e0u32;
        let mc = llvm_mc_word("ngc x0, x1").expect("llvm-mc KAT ngc");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken for ngc");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("xzr".into()),
            Operand::Reg("x1".into()),
        ];
        let sut = sut_word(&ops, false).expect("SUT KAT ngc alias");
        assert_eq!(sut, want);
    }
    ...
    proptest! {
        ...
        #[test]
        fn encode_sbc_diff_gpr_same_width(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            is_64 in any::<bool>(),
            ...
            let sut = sut_word(&ops, set_flags)
                .unwrap_or_else(|e| panic!("SUT rejected valid SBC {}: {}", asm, e));
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid SBC {}: {}", asm, e));
            prop_assert_eq!(sut, mc, "SUT vs llvm-mc mismatch for {}", asm);
        }

        #[test]
        fn encode_sbc_meta_s_bit(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            is_64 in any::<bool>(),
            ...
            let sbc = sut_word(&ops, false)
                .unwrap_or_else(|e| panic!("sbc rejected: {}", e));
            let sbcs = sut_word(&ops, true)
                .unwrap_or_else(|e| panic!("sbcs rejected: {}", e));
            prop_assert_eq!(
                sbc ^ sbcs,
                1u32 << 29,
                "SBC vs SBCS must differ only by S bit 29 (sbc={:#010x} sbcs={:#010x})",
                sbc,
                sbcs
            );
        }
        ...
        fn encode_sbc_meta_vs_adc(
            ...
            let sbc = sut_word(&ops, set_flags)
                .unwrap_or_else(|e| panic!("sbc rejected: {}", e));
            let adc = adc_word(&ops, set_flags)
                .unwrap_or_else(|e| panic!("adc rejected: {}", e));
            prop_assert_eq!(
                sbc ^ adc,
                1u32 << 30,
                "SBC vs ADC must differ only by op bit 30 (sbc={:#010x} adc={:#010x})",
                sbc,
                adc
            );
        }
        ...
        #[test]
        fn encode_sbc_invariant_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            is_64 in any::<bool>(),
            ...
            prop_assert_eq!((w >> 31) & 1, sf, "sf bit");
            prop_assert_eq!((w >> 29) & 1, s, "S bit");
            prop_assert_eq!((w >> 30) & 1, 1, "op bit must be 1 for SBC");
            prop_assert_eq!((w >> 21) & 0xFF, 0b11010000u32, "opcode bits 28:21");
            prop_assert_eq!((w >> 10) & 0x3F, 0, "bits 15:10 must be 000000");
        }

        #[test]
        fn encode_sbc_meta_ngc_alias(
            rd in 0u32..=31,
            rm in 0u32..=31,
            is_64 in any::<bool>(),
            set_flags in any::<bool>(),
            ...
            let ngc_asm = format!(
                "{} {}, {}",
                ngc_mnemonic(set_flags), rd_n, rm_n
            );
            let ops = [
                Operand::Reg(rd_n),
                Operand::Reg(zr),
            ...
            let sut = sut_word(&ops, set_flags)
                .unwrap_or_else(|e| panic!("SUT rejected NGC-alias SBC: {}", e));
            let mc = llvm_mc_word(&ngc_asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid NGC {}: {}", ngc_asm, e));
            prop_assert_eq!(sut, mc, "SBC Rd, ZR, Rm vs llvm-mc {} mismatch", ngc_asm);
        }

        #[test]
        fn encode_sbc_meta_lr_alias(
            which in 0u32..=2,
            a in 0u32..=30,
            b in 0u32..=30,
            set_flags in any::<bool>(),
            ...
            let xa = format!("x{}", a);
            let xb = format!("x{}", b);
            let mut names_lr = [xa.clone(), xb.clone(), "lr".to_string()];
            names_lr.swap(2, which as usize);
            let mut names_x30 = [xa, xb, "x30".to_string()];
            names_x30.swap(2, which as usize);
            let ops_lr = [
                Operand::Reg(names_lr[0].clone()),
                Operand::Reg(names_lr[1].clone()),
                Operand::Reg(names_lr[2].clone()),
            ];
            let ops_x30 = [
                Operand::Reg(names_x30[0].clone()),
                Operand::Reg(names_x30[1].clone()),
                Operand::Reg(names_x30[2].clone()),
            ];
            let sut_lr = sut_word(&ops_lr, set_flags)
                .unwrap_or_else(|e| panic!("sbc with lr rejected: {}", e));
            let sut_x30 = sut_word(&ops_x30, set_flags)
                .unwrap_or_else(|e| panic!("sbc with x30 rejected: {}", e));
            prop_assert_eq!(sut_lr, sut_x30, "lr must encode as x30");
            let asm = format!(
                "{} {}, {}, {}",
                mnemonic(set_flags), names_lr[0], names_lr[1], names_lr[2]
            );
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected {}: {}", asm, e));
            prop_assert_eq!(sut_lr, mc, "SUT vs llvm-mc mismatch for {}", asm);
        }

        #[test]
        fn encode_sbc_neg_too_few_operands(
            n in 0usize..=2,
            set_flags in any::<bool>(),
            is_64 in any::<bool>(),
            r0 in 0u32..=31,
        ...
        #[test]
        fn encode_sbc_neg_non_register(
            rd in 0u32..=30,
            rn in 0u32..=30,
            is_64 in any::<bool>(),
            set_flags in any::<bool>(),
        ...
        #[test]
        fn encode_sbc_neg_invalid_reg_name(
            which in 0u32..=2,
            set_flags in any::<bool>(),
            bad in prop_oneof![
                Just("x32".to_string()),
        ...
        fn encode_sbc_neg_fp_reg(
            ...
            prop_assert!(
                encode_sbc(&ops, set_flags).is_err(),
                "FP/SIMD register {} is not a valid SBC operand (which={})",
                fp,
                which
            );
        }
        ...
        fn encode_sbc_neg_extra_shift(
            ...
            prop_assert!(
                encode_sbc(&ops, set_flags).is_err(),
                "SBC has no shifted-register form; extra {} #{} must Err",
                kind,
                amt
            );
        }
        ...
        fn encode_sbc_neg_mixed_width(
            ...
            prop_assert!(
                encode_sbc(&ops, set_flags).is_err(),
                "mixed-width SBC registers must Err (rd64={} rn64={} rm64={})",
                rd64,
                rn64,
                rm64
            );
        ...
        fn encode_sbc_neg_sp(
            ...
            prop_assert!(
                encode_sbc(&ops, set_flags).is_err(),
                "SP/WSP is not a valid SBC operand (which={} names={:?})",
                which,
                names
            );
        }
    ...
    #[test]
    fn test_encode_sbc_regression_extra_shift() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_sbc(&ops, false).is_err(),
            "SBC has no shifted-register form; extra lsl #0 must Err (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_sbc_regression_mixed_width() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("x0".into()),
        ...
        assert!(
            encode_sbc(&ops, false).is_err(),
            "mixed-width SBC w0, w0, x0 must Err (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_sbc_regression_sp() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_sbc(&ops, false).is_err(),
            "SBC wsp, w0, w0 must Err; register 31 is WZR not WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_sbc_regression_fp_reg() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("x1".into()),
            Operand::Reg("x2".into()),
        ...
        assert!(
            encode_sbc(&ops, false).is_err(),
            "SBC d0, x1, x2 must Err; FP/SIMD registers are not SBC operands (llvm-mc rejects it)"
        );
    }
}

#[cfg(test)]
mod encode_smull_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:247-256 smull dispatch (scalar path); ARM ARM Data-processing (3 source)
    //   SMULL Xd, Wn, Wm is the alias of SMADDL Xd, Wn, Wm, XZR:
    //   sf=1 U=0 11011 001 Rm o0=0 Ra=11111 Rn Rd;
    //   data_processing.rs:630 docstring; README.md:214 lists smull
    // Stronger considered:
    //   - State machine: rejected — encode_smull is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree SMULL decoder
    //   - encode_umull as differential sibling: rejected — same-job gate fails (U=1 unsigned)
    //   - encode_smaddl as independent differential: rejected — shared get_reg / same TU;
    //     alias equality is algebraic.metamorphic, not an independent implementation
    // Weaker available: algebraic.metamorphic (SMADDL Ra=XZR alias, U bit vs UMULL),
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / wrong width / SP / FP)
    // Differential: candidate=encode_smull, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=operands <-> asm text `smull Xd, Wn, Wm`

    use super::{encode_smaddl, encode_smull, encode_umull};
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    const LLVM_MC: &str = "/home/toan/tools/llvm15-official/bin/llvm-mc";
    const CASES: u32 = 1000;
    const SMULL_BASE: u32 = 0x9B20_7C00; // sf=1 U=0 o0=0 Ra=XZR, Rd=Rn=Rm=0
    ...
    fn smaddl_word(ops: &[Operand]) -> Result<u32, String> {
    ...
    fn umull_word(ops: &[Operand]) -> Result<u32, String> {
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_smull_kat_llvm_mc_x0_w1_w2() {
        let want = 0x9b227c20u32;
        let mc = llvm_mc_word("smull x0, w1, w2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("w1".into()),
    ...
    #[test]
    fn encode_smull_kat_llvm_mc_xzr_wzr_wzr() {
        let want = 0x9b3f7fffu32;
        let mc = llvm_mc_word("smull xzr, wzr, wzr").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("xzr".into()),
    ...
    #[test]
    fn encode_smull_kat_llvm_mc_alias_smaddl_xzr() {
        let want = 0x9b227c20u32;
    ...
    #[test]
    fn encode_smull_kat_llvm_mc_lr_w1_w2() {
        let want = 0x9b227c3eu32;
        let mc = llvm_mc_word("smull lr, w1, w2").expect("llvm-mc LR KAT");
    ...
    proptest! {
        ...
        #[test]
        fn encode_smull_diff_valid_gpr(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            use_lr in any::<bool>(),
            ...
            let src_n = wreg(rn);
            let src_m = wreg(rm);
            let asm = format!("smull {}, {}, {}", dest, src_n, src_m);
            let ops = [
                Operand::Reg(dest),
                Operand::Reg(src_n),
                Operand::Reg(src_m),
            ...
            prop_assert_eq!(sut, mc, "SMULL mismatch for {}", asm);
        }

        #[test]
        fn encode_smull_alias_smaddl_xzr(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
        ) {
            ...
            let sut = sut_word(&smull_ops).expect("SUT SMULL");
            let alias = smaddl_word(&smaddl_ops).expect("SUT SMADDL XZR");
            prop_assert_eq!(sut, alias, "SMULL must equal SMADDL with Ra=XZR");
        ...
        #[test]
        fn encode_smull_xor_umull_u_bit(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
        ) {
            ...
            let s = sut_word(&ops).expect("SMULL");
            let u = umull_word(&ops).expect("UMULL");
            prop_assert_eq!(s ^ u, 1u32 << 23, "SMULL XOR UMULL must be U bit 23");
        }

        #[test]
        fn encode_smull_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
        ) {
            ...
            let want = SMULL_BASE | (rm << 16) | (rn << 5) | rd;
            prop_assert_eq!(w, want, "ARM ARM SMULL field layout");
        ...
        #[test]
        fn encode_smull_diff_alt_spellings(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ...
            // Cover x31/w31 (not just xzr/wzr), uppercase, and LR — names get_reg accepts
            // that the first differential generator under-sampled.
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "SMULL alt-spelling mismatch for {}", asm);
        }

        #[test]
        fn encode_smull_neg_arity(
            len in 0usize..=2,
            a in 0u32..=31,
            b in 0u32..=31,
            ...
            prop_assert!(
                encode_smull(&ops).is_err(),
                "SMULL with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_smull_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            extra in extra_operand(),
            ...
            prop_assert!(
                encode_smull(&ops).is_err(),
                "SMULL has no 4th operand; extra operand must Err (llvm-mc rejects it)"
            );
        }

        #[test]
        fn encode_smull_neg_wrong_width(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            rd64 in any::<bool>(),
            ...
            prop_assert!(
                encode_smull(&ops).is_err(),
                "SMULL requires Xd, Wn, Wm; rd64={} rn64={} rm64={} must Err (llvm-mc rejects it)",
                rd64,
                rn64,
                rm64
            );
        ...
        fn encode_smull_neg_sp(
            ...
            prop_assert!(
                encode_smull(&ops).is_err(),
                "SP/WSP is not a valid SMULL operand (which={} names={:?})",
                which,
                names
            );
        }
        ...
        fn encode_smull_neg_fp(
            ...
            prop_assert!(
                encode_smull(&ops).is_err(),
                "FP/SIMD register {} is not a valid SMULL operand (which={})",
                fp,
                which
            );
        }
        ...
        #[test]
        fn encode_smull_neg_invalid_name(
            which in 0u32..=2,
            name in invalid_name(),
        ) {
            let mut ops = vec![
    ...
    #[test]
    fn test_encode_smull_regression_extra_operand() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_smull(&ops).is_err(),
            "SMULL x0, w0, w0, x0 must Err; extra operand is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_smull_regression_wrong_width() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_smull(&ops).is_err(),
            "SMULL w0, w0, w0 must Err; dest must be Xd (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_smull_regression_sp() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_smull(&ops).is_err(),
            "SMULL wsp, w0, w0 must Err; register 31 is WZR not WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_smull_regression_fp() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("w1".into()),
            Operand::Reg("w2".into()),
        ...
        assert!(
            encode_smull(&ops).is_err(),
            "SMULL d0, w1, w2 must Err; FP/SIMD registers are not SMULL operands (llvm-mc rejects it)"
        );
    }
}
...
mod encode_sxth_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:294 "sxth" => encode_sxth; README.md:217 Extensions lists sxth;
    //   ARM ARM C6 SXTH is the alias of SBFM Rd, Rn, #0, #15:
    //   sf 00 100110 N=sf immr=0 imms=15 Rn Rd
    // Stronger considered:
    //   - State machine: rejected — encode_sxth is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree SXTH/SBFM decoder
    //   - encode_sxtb / encode_uxth as differential siblings: rejected — same-job gate fails
    //     (imms=7 / UBFM opc=10)
    //   - encode_sbfm as independent differential: rejected — shared get_reg / same crate;
    //     alias equality is algebraic.metamorphic, not an independent implementation
    // Weaker available: algebraic.metamorphic (SBFM #0,#15 alias), algebraic.invariant (ARM fields),
    //   negative_error (arity / extra / Wd+Xn / SP / FP)
    // Differential: candidate=encode_sxth, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=operands <-> asm text `sxth Rd, Rn`

    use super::encode_sxth;
    ...
    const LLVM_MC: &str = "/home/toan/tools/llvm15-official/bin/llvm-mc";
    const CASES: u32 = 1000;
    const SXTH32_BASE: u32 = 0x1300_3C00; // sf=0 N=0 immr=0 imms=15, Rd=Rn=0
    const SXTH64_BASE: u32 = 0x9340_3C00; // sf=1 N=1 immr=0 imms=15, Rd=Rn=0
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_sxth_kat_llvm_mc_w0_w1() {
        let want = 0x1300_3c20u32;
        let mc = llvm_mc_word("sxth w0, w1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("w0".into()), Operand::Reg("w1".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_sxth_kat_llvm_mc_x0_w1() {
        let want = 0x9340_3c20u32;
        let mc = llvm_mc_word("sxth x0, w1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
    ...
    #[test]
    fn encode_sxth_kat_llvm_mc_wzr_wzr() {
        let want = 0x1300_3fffu32;
        let mc = llvm_mc_word("sxth wzr, wzr").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
    ...
    #[test]
    fn encode_sxth_kat_llvm_mc_alias_sbfm() {
        let want = 0x1300_3c20u32;
        let mc_sxth = llvm_mc_word("sxth w0, w1").expect("llvm-mc SXTH KAT");
        let mc_sbfm = llvm_mc_word("sbfm w0, w1, #0, #15").expect("llvm-mc SBFM KAT");
        assert_eq!(mc_sxth, want, "llvm-mc SXTH KAT mapping broken");
    ...
    proptest! {
        ...
        fn encode_sxth_diff_valid_gpr(
            ...
            let src = wreg(rn);
            let asm = format!("sxth {}, {}", dest, src);
            let ops = [Operand::Reg(dest), Operand::Reg(src)];
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "SXTH mismatch for {}", asm);
        }

        #[test]
        fn encode_sxth_alias_sbfm(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_64 in any::<bool>(),
        ) {
            ...
            let sxth_ops = [Operand::Reg(dest.clone()), Operand::Reg(src.clone())];
            let sbfm_ops = [
                Operand::Reg(dest.clone()),
            ...
            let sut = sut_word(&sxth_ops).expect("SUT SXTH");
            let alias = sbfm_word(&sbfm_ops).expect("SUT SBFM #0,#15");
            prop_assert_eq!(sut, alias, "SXTH must equal SBFM with #0,#15");
            let mc_src = wreg(rn);
            let mc = llvm_mc_word(&format!("sxth {}, {}", dest, mc_src)).expect("llvm-mc SXTH");
            prop_assert_eq!(sut, mc);
        }

        #[test]
        fn encode_sxth_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_64 in any::<bool>(),
        ) {
            ...
            let w = sut_word(&ops).expect("SUT");
            let base = if is_64 { SXTH64_BASE } else { SXTH32_BASE };
            let want = base | (rn << 5) | rd;
            prop_assert_eq!(w, want, "ARM ARM SXTH field layout");
        ...
        fn encode_sxth_neg_arity(
            ...
            prop_assert!(
                ...
                "SXTH with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_sxth_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_64 in any::<bool>(),
            extra in extra_operand(),
            ...
            prop_assert!(
                encode_sxth(&ops).is_err(),
                "SXTH has no 3rd operand; extra operand must Err (llvm-mc rejects it)"
            );
        }

        #[test]
        fn encode_sxth_neg_wd_xn(
            rd in 0u32..=31,
            rn in 0u32..=31,
        ) {
            ...
            prop_assert!(
                encode_sxth(&ops).is_err(),
                "SXTH Wd, Xn must Err (llvm-mc rejects sxth {}, {})",
                wreg(rd),
                xreg(rn)
            );
        }
        ...
        fn encode_sxth_neg_sp(
            ...
            prop_assert!(
                encode_sxth(&ops).is_err(),
                "SP/WSP is not a valid SXTH operand (which={} names={:?})",
                which,
                names
            );
        }
        ...
        fn encode_sxth_neg_fp(
            ...
            prop_assert!(
                encode_sxth(&ops).is_err(),
                "FP/SIMD register {} is not a valid SXTH operand (which={})",
                fp,
                which
            );
        }
        ...
        #[test]
        fn encode_sxth_diff_alt_spellings(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_64 in any::<bool>(),
            dest_spell in 0u32..=4,
            ...
            // (llvm-mc canonicalizes sxth Xd, Xn to sxth Xd, Wn).
            let dest = match dest_spell {
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "SXTH alt-spelling mismatch for {}", asm);
        }

        #[test]
        fn encode_sxth_neg_nonreg(
            which in 0u32..=1,
            bad in non_reg_operand(),
        ) {
            let mut ops = vec![
        ...
        #[test]
        fn encode_sxth_neg_invalid_name(
            which in 0u32..=1,
            name in invalid_name(),
        ) {
            let mut ops = vec![
    ...
    #[test]
    fn test_encode_sxth_regression_extra_operand() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("x0".into()),
        ...
        assert!(
            encode_sxth(&ops).is_err(),
            "SXTH w0, w0, x0 must Err; extra operand is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_sxth_regression_wd_xn() {
        let ops = [Operand::Reg("w0".into()), Operand::Reg("x0".into())];
        assert!(
            encode_sxth(&ops).is_err(),
            "SXTH w0, x0 must Err; 32-bit dest requires Wn source (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_sxth_regression_sp() {
        let ops = [Operand::Reg("wsp".into()), Operand::Reg("w0".into())];
        assert!(
            encode_sxth(&ops).is_err(),
            "SXTH wsp, w0 must Err; register 31 is WZR not WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_sxth_regression_fp() {
        let ops = [Operand::Reg("d0".into()), Operand::Reg("w1".into())];
        assert!(
            encode_sxth(&ops).is_err(),
            "SXTH d0, w1 must Err; FP/SIMD registers are not SXTH operands (llvm-mc rejects it)"
        );
    }
}
...
#[cfg(test)]
mod encode_umaddl_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:270 umaddl dispatch; ARM ARM Data-processing (3 source)
    //   UMADDL Xd, Wn, Wm, Xa: sf=1 U=1 11011 101 Rm o0=0 Ra Rn Rd;
    //   UMULL Xd, Wn, Wm is the alias of UMADDL Xd, Wn, Wm, XZR;
    //   data_processing.rs:664 docstring; README.md:214 lists umaddl
    // Stronger considered:
    //   - State machine: rejected — encode_umaddl is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree UMADDL decoder
    //   - encode_smaddl as differential sibling: rejected — same-job gate fails (U=0 signed)
    //   - encode_umull as independent differential: rejected — shared get_reg / same TU;
    //     alias equality is algebraic.metamorphic, not an independent implementation
    // Weaker available: algebraic.metamorphic (UMULL Ra=XZR alias, U bit vs SMADDL),
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / wrong width / SP / FP)
    // Differential: candidate=encode_umaddl, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=operands <-> asm text `umaddl Xd, Wn, Wm, Xa`

    use super::{encode_smaddl, encode_umaddl, encode_umull};
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    const LLVM_MC: &str = "/home/toan/tools/llvm15-official/bin/llvm-mc";
    const CASES: u32 = 1000;
    const UMADDL_BASE: u32 = 0x9BA0_0000; // sf=1 U=1 o0=0, Rd=Rn=Rm=Ra=0
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_umaddl_kat_llvm_mc_x0_w1_w2_x3() {
        let want = 0x9ba20c20u32;
        let mc = llvm_mc_word("umaddl x0, w1, w2, x3").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("w1".into()),
    ...
    #[test]
    fn encode_umaddl_kat_llvm_mc_xzr_wzr_wzr_xzr() {
        let want = 0x9bbf7fffu32;
        let mc = llvm_mc_word("umaddl xzr, wzr, wzr, xzr").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("xzr".into()),
            Operand::Reg("wzr".into()),
    ...
    #[test]
    fn encode_umaddl_kat_llvm_mc_alias_umull_xzr() {
    ...
    #[test]
    fn encode_umaddl_kat_llvm_mc_lr_w1_w2_x30() {
        let want = 0x9ba2783eu32;
        let mc = llvm_mc_word("umaddl lr, w1, w2, x30").expect("llvm-mc LR KAT");
        assert_eq!(mc, want, "llvm-mc LR KAT mapping broken");
        let ops = [
            Operand::Reg("lr".into()),
            Operand::Reg("w1".into()),
    ...
    proptest! {
        ...
        #[test]
        fn encode_umaddl_diff_valid_gpr(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            let asm = format!("umaddl {}, {}, {}, {}", dest, src_n, src_m, acc);
            let ops = [
                Operand::Reg(dest),
                Operand::Reg(src_n),
                Operand::Reg(src_m),
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UMADDL mismatch for {}", asm);
        }

        #[test]
        fn encode_umaddl_alias_umull_xzr(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
        ) {
            ...
            let sut = sut_word(&umaddl_ops).expect("SUT UMADDL XZR");
            let alias = umull_word(&umull_ops).expect("SUT UMULL");
            prop_assert_eq!(sut, alias, "UMADDL with Ra=XZR must equal UMULL");
        ...
        #[test]
        fn encode_umaddl_xor_smaddl_u_bit(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            let u = sut_word(&ops).expect("UMADDL");
            let s = smaddl_word(&ops).expect("SMADDL");
            prop_assert_eq!(u ^ s, 1u32 << 23, "UMADDL XOR SMADDL must be U bit 23");
        }

        #[test]
        fn encode_umaddl_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            let w = sut_word(&ops).expect("SUT");
            let want = UMADDL_BASE | (rm << 16) | (ra << 10) | (rn << 5) | rd;
            prop_assert_eq!(w, want, "ARM ARM UMADDL field layout");
            prop_assert_eq!(w >> 31, 1, "sf must be 1");
        ...
        fn encode_umaddl_neg_arity(
            ...
            prop_assert!(
                encode_umaddl(&ops).is_err(),
                "UMADDL with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_umaddl_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            prop_assert!(
                encode_umaddl(&ops).is_err(),
                "UMADDL has no 5th operand; extra operand must Err (llvm-mc rejects it)"
            );
        }

        #[test]
        fn encode_umaddl_neg_wrong_width(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            ra in 0u32..=30,
            ...
            prop_assert!(
                encode_umaddl(&ops).is_err(),
                "UMADDL requires Xd, Wn, Wm, Xa; rd64={} rn64={} rm64={} ra64={} must Err (llvm-mc rejects it)",
                rd64,
                rn64,
                rm64,
                ra64
        ...
        fn encode_umaddl_neg_sp(
            ...
            prop_assert!(
                encode_umaddl(&ops).is_err(),
                "SP/WSP is not a valid UMADDL operand (which={} names={:?})",
                which,
                names
            );
        }
        ...
        #[test]
        fn encode_umaddl_diff_alt_spellings(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UMADDL alt-spelling mismatch for {}", asm);
        }

        #[test]
        fn encode_umaddl_neg_fp(
            which in 0u32..=3,
            prefix in prop::sample::select(vec!["d", "s", "q", "v", "h", "b"]),
            n in prop_oneof![Just(0u32), Just(31u32), 0u32..=31],
        ) {
            ...
            prop_assert!(
                encode_umaddl(&ops).is_err(),
                "FP/SIMD register {} is not a valid UMADDL operand (which={})",
                fp,
                which
            );
        }
        ...
        #[test]
        fn encode_umaddl_neg_invalid_name(
            which in 0u32..=3,
            name in invalid_name(),
        ) {
            let mut ops = vec![
    ...
    #[test]
    fn test_encode_umaddl_regression_extra_operand() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_umaddl(&ops).is_err(),
            "UMADDL x0, w0, w0, x0, x0 must Err; extra operand is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_umaddl_regression_wrong_width() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_umaddl(&ops).is_err(),
            "UMADDL w0, w0, w0, w0 must Err; dest must be Xd and acc must be Xa (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_umaddl_regression_sp() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_umaddl(&ops).is_err(),
            "UMADDL wsp, w0, w0, x0 must Err; register 31 is WZR not WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_umaddl_regression_fp() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("w1".into()),
            Operand::Reg("w2".into()),
        ...
        assert!(
            encode_umaddl(&ops).is_err(),
            "UMADDL d0, w1, w2, x3 must Err; FP/SIMD registers are not UMADDL operands (llvm-mc rejects it)"
        );
    }
}

#[cfg(test)]
mod encode_umulh_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:274 umulh dispatch; ARM ARM Data-processing (3 source)
    //   UMULH Xd, Xn, Xm: sf=1 op54=00 11011 op31=110 Rm o0=0 Ra=11111 Rn Rd;
    //   data_processing.rs:688-694; README.md:214 lists umulh
    // Stronger considered:
    //   - State machine: rejected — encode_umulh is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree UMULH decoder
    //   - encode_smulh as differential sibling: rejected — same-job gate fails (U=0 signed)
    // Weaker available: algebraic.metamorphic (U bit vs SMULH),
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / wrong width / SP)
    // Differential: candidate=encode_umulh, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=operands <-> asm text `umulh Xd, Xn, Xm`
    ...
    const LLVM_MC: &str = "/home/toan/tools/llvm15-official/bin/llvm-mc";
    const CASES: u32 = 1000;
    const UMULH_BASE: u32 = 0x9BC0_7C00; // sf=1 op31=110 o0=0 Ra=31, Rd=Rn=Rm=0
    ...
    fn smulh_word(ops: &[Operand]) -> Result<u32, String> {
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_umulh_kat_llvm_mc_x0_x1_x2() {
        let want = 0x9bc27c20u32;
        let mc = llvm_mc_word("umulh x0, x1, x2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("x1".into()),
    ...
    #[test]
    fn encode_umulh_kat_llvm_mc_xzr_xzr_xzr() {
        let want = 0x9bdf7fffu32;
        let mc = llvm_mc_word("umulh xzr, xzr, xzr").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("xzr".into()),
            Operand::Reg("xzr".into()),
    ...
    #[test]
    fn encode_umulh_kat_llvm_mc_lr_x1_x30() {
        let want = 0x9bde7c3eu32;
        let mc = llvm_mc_word("umulh lr, x1, x30").expect("llvm-mc LR KAT");
        assert_eq!(mc, want, "llvm-mc LR KAT mapping broken");
        let ops = [
            Operand::Reg("lr".into()),
            Operand::Reg("x1".into()),
    ...
    #[test]
    fn encode_umulh_kat_llvm_mc_x0_x1_xzr() {
        let want = 0x9bdf7c20u32;
        let mc = llvm_mc_word("umulh x0, x1, xzr").expect("llvm-mc XZR Rm KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("x1".into()),
    ...
    proptest! {
        ...
        #[test]
        fn encode_umulh_diff_valid_gpr(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            use_lr in any::<bool>(),
            ...
            let asm = format!("umulh {}, {}, {}", dest, src_n, src_m);
            let ops = [
                Operand::Reg(dest),
                Operand::Reg(src_n),
                Operand::Reg(src_m),
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UMULH mismatch for {}", asm);
        }

        #[test]
        fn encode_umulh_xor_smulh_u_bit(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
        ) {
            ...
            let u = sut_word(&ops).expect("UMULH");
            let s = smulh_word(&ops).expect("SMULH");
            prop_assert_eq!(u ^ s, 1u32 << 23, "UMULH XOR SMULH must be U bit 23");
        }

        #[test]
        fn encode_umulh_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
        ) {
            ...
            let w = sut_word(&ops).expect("SUT");
            let want = UMULH_BASE | (rm << 16) | (rn << 5) | rd;
            prop_assert_eq!(w, want, "ARM ARM UMULH field layout");
            prop_assert_eq!(w >> 31, 1, "sf must be 1");
            prop_assert_eq!((w >> 21) & 0x3ff, 0b00_11011_110, "bits[30:21]");
            prop_assert_eq!((w >> 16) & 0x1f, rm, "Rm");
            prop_assert_eq!((w >> 15) & 1, 0, "o0 must be 0");
        ...
        fn encode_umulh_neg_arity(
            ...
            prop_assert!(
                ...
                "UMULH with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_umulh_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            extra in extra_operand(),
            ...
            prop_assert!(
                encode_umulh(&ops).is_err(),
                "UMULH has no 4th operand; extra operand must Err (llvm-mc rejects it)"
            );
        }

        #[test]
        fn encode_umulh_neg_wrong_width(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            rd64 in any::<bool>(),
            ...
            prop_assert!(
                encode_umulh(&ops).is_err(),
                "UMULH requires Xd, Xn, Xm; rd64={} rn64={} rm64={} must Err (llvm-mc rejects it)",
                rd64,
                rn64,
                rm64
            );
        ...
        fn encode_umulh_neg_sp(
            ...
            prop_assert!(
                encode_umulh(&ops).is_err(),
                "SP/WSP is not a valid UMULH operand (which={} names={:?})",
                which,
                names
            );
        }
        ...
        #[test]
        fn encode_umulh_diff_alt_spellings(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            dest_spell in 0u32..=4,
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UMULH alt-spelling mismatch for {}", asm);
        }

        #[test]
        fn encode_umulh_neg_fp(
            which in 0u32..=2,
            prefix in prop::sample::select(vec!["d", "s", "q", "v", "h", "b"]),
            n in prop_oneof![Just(0u32), Just(31u32), 0u32..=31],
        ) {
            ...
            prop_assert!(
                encode_umulh(&ops).is_err(),
                "FP/SIMD register {} is not a valid UMULH operand (which={})",
                fp,
                which
            );
        }
        ...
        #[test]
        fn encode_umulh_neg_invalid_name(
            which in 0u32..=2,
            name in invalid_name(),
        ) {
            let mut ops = vec![
    ...
    #[test]
    fn test_encode_umulh_regression_extra_operand() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("x0".into()),
            Operand::Reg("x0".into()),
        ...
        assert!(
            encode_umulh(&ops).is_err(),
            "UMULH x0, x0, x0, x0 must Err; extra operand is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_umulh_regression_wrong_width() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("w0".into()),
        ...
        assert!(
            encode_umulh(&ops).is_err(),
            "UMULH w0, w0, w0 must Err; all three operands must be 64-bit X registers (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_umulh_regression_sp() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Reg("x0".into()),
            Operand::Reg("x0".into()),
        ...
        assert!(
            encode_umulh(&ops).is_err(),
            "UMULH wsp, x0, x0 must Err; register 31 is XZR/WZR not SP/WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_umulh_regression_fp() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("x1".into()),
            Operand::Reg("x2".into()),
        ...
        assert!(
            encode_umulh(&ops).is_err(),
            "UMULH d0, x1, x2 must Err; FP/SIMD registers are not UMULH operands (llvm-mc rejects it)"
        );
    }
}
...
mod encode_uxtw_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:296 "uxtw" => encode_uxtw; README.md:217 Extensions lists uxtw;
    //   ARM ARM C6 UXTW is the alias of UBFM Xd, Xn, #0, #31:
    //   sf=1 10 100110 N=1 immr=0 imms=31 Rn Rd. Assembler syntax: UXTW Xd, Wn only.
    // Stronger considered:
    //   - State machine: rejected — encode_uxtw is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree UXTW/UBFM decoder
    //   - encode_sxtw / encode_uxth / encode_uxtb as differential siblings: rejected — same-job gate fails
    //     (SBFM signed / imms=15 / imms=7)
    //   - encode_ubfm as independent differential: rejected — shared get_reg / same crate;
    //     alias equality is algebraic.metamorphic, not an independent implementation
    // Weaker available: algebraic.metamorphic (UBFM #0,#31 alias), algebraic.invariant (ARM fields),
    //   negative_error (arity / extra / Wd / SP / FP)
    // Differential: candidate=encode_uxtw, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=operands <-> asm text `uxtw Rd, Rn`

    use super::encode_uxtw;
    use super::super::{encode_ubfm, EncodeResult};
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    use std::process::{Command, Stdio};
    ...
    const LLVM_MC: &str = "/home/toan/tools/llvm15-official/bin/llvm-mc";
    const CASES: u32 = 1000;
    const UXTW_BASE: u32 = 0xD340_7C00; // sf=1 opc=10 N=1 immr=0 imms=31, Rd=Rn=0
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_uxtw_kat_llvm_mc_x0_w1() {
        let want = 0xD340_7c20u32;
        let mc = llvm_mc_word("uxtw x0, w1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("x0".into()), Operand::Reg("w1".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_uxtw_kat_llvm_mc_xzr_wzr() {
        let want = 0xD340_7fffu32;
        let mc = llvm_mc_word("uxtw xzr, wzr").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("xzr".into()), Operand::Reg("wzr".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_uxtw_kat_llvm_mc_lr_w0() {
        let want = 0xD340_7c1eu32;
        let mc = llvm_mc_word("uxtw lr, w0").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("lr".into()), Operand::Reg("w0".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_uxtw_kat_llvm_mc_alias_ubfm() {
        let want = 0xD340_7c20u32;
        let mc_uxtw = llvm_mc_word("uxtw x0, w1").expect("llvm-mc UXTW KAT");
        let mc_ubfm = llvm_mc_word("ubfm x0, x1, #0, #31").expect("llvm-mc UBFM KAT");
        assert_eq!(mc_uxtw, want, "llvm-mc UXTW KAT mapping broken");
    ...
    proptest! {
        ...
        fn encode_uxtw_diff_valid_gpr(
            ...
            let src = wreg(rn);
            let asm = format!("uxtw {}, {}", dest, src);
            let ops = [Operand::Reg(dest), Operand::Reg(src)];
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UXTW mismatch for {}", asm);
        }

        #[test]
        fn encode_uxtw_alias_ubfm(
            rd in 0u32..=31,
            rn in 0u32..=31,
        ) {
            let dest = xreg(rd);
            let src_w = wreg(rn);
            let src_x = xreg(rn);
            let uxtw_ops = [Operand::Reg(dest.clone()), Operand::Reg(src_w.clone())];
            let ubfm_ops = [
                Operand::Reg(dest.clone()),
                Operand::Reg(src_x),
                Operand::Imm(0),
            ...
            let sut = sut_word(&uxtw_ops).expect("SUT UXTW");
            let alias = ubfm_word(&ubfm_ops).expect("SUT UBFM #0,#31");
            prop_assert_eq!(sut, alias, "UXTW must equal UBFM with #0,#31");
            let mc = llvm_mc_word(&format!("uxtw {}, {}", dest, src_w)).expect("llvm-mc UXTW");
            prop_assert_eq!(sut, mc);
        }

        #[test]
        fn encode_uxtw_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
        ) {
            let ops = [Operand::Reg(xreg(rd)), Operand::Reg(wreg(rn))];
            let w = sut_word(&ops).expect("SUT");
            let want = UXTW_BASE | (rn << 5) | rd;
            prop_assert_eq!(w, want, "ARM ARM UXTW field layout");
            prop_assert_eq!(w >> 31, 1, "sf must be 1");
        ...
        fn encode_uxtw_neg_arity(
            ...
            prop_assert!(
                ...
                "UXTW with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_uxtw_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            extra in extra_operand(),
        ) {
            ...
            prop_assert!(
                encode_uxtw(&ops).is_err(),
                "UXTW has no 3rd operand; extra operand must Err (llvm-mc rejects it)"
            );
        }

        #[test]
        fn encode_uxtw_neg_wd(
            rd in 0u32..=31,
            rn in 0u32..=31,
            src64 in any::<bool>(),
        ) {
            ...
            prop_assert!(
                encode_uxtw(&ops).is_err(),
                "UXTW Wd, {} must Err (llvm-mc rejects uxtw {}, {})",
                src,
                dest,
                src
            );
        ...
        fn encode_uxtw_neg_sp(
            ...
            prop_assert!(
                encode_uxtw(&ops).is_err(),
                "SP/WSP is not a valid UXTW operand (which={} names={:?})",
                which,
                names
            );
        }
        ...
        fn encode_uxtw_neg_fp(
            ...
            prop_assert!(
                encode_uxtw(&ops).is_err(),
                "FP/SIMD register {} is not a valid UXTW operand (which={})",
                fp,
                which
            );
        }
        ...
        #[test]
        fn encode_uxtw_diff_alt_spellings(
            rd in 0u32..=31,
            rn in 0u32..=31,
            dest_spell in 0u32..=4,
            src_spell in 0u32..=3,
            ...
            // Cover x31 (not just xzr), uppercase, LR, and X-source
            // (llvm-mc canonicalizes uxtw Xd, Xn to ubfx Xd, Xn, #0, #32).
            let dest = match dest_spell {
                0 if rd == 31 => "x31".to_string(),
                1 if rd == 31 => "XZR".to_string(),
                2 if rd == 30 => "LR".to_string(),
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UXTW alt-spelling mismatch for {}", asm);
        }

        #[test]
        fn encode_uxtw_neg_nonreg(
            which in 0u32..=1,
            bad in non_reg_operand(),
        ) {
            let mut ops = vec![
        ...
        #[test]
        fn encode_uxtw_neg_invalid_name(
            which in 0u32..=1,
            name in invalid_name(),
        ) {
            let mut ops = vec![
    ...
    #[test]
    fn test_encode_uxtw_regression_mov_not_ubfm() {
        let ops = [Operand::Reg("x0".into()), Operand::Reg("w1".into())];
        match encode_uxtw(&ops) {
            Ok(EncodeResult::Word(w)) => {
                assert_eq!(
                    w, 0xD340_7c20u32,
                    "UXTW x0, w1 must encode as UBFM Xd,Xn,#0,#31 (0xD3407C20), not MOV Wd,Wn"
                );
            }
    ...
    #[test]
    fn test_encode_uxtw_regression_extra_operand() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("w0".into()),
            Operand::Reg("x0".into()),
        ...
        assert!(
            encode_uxtw(&ops).is_err(),
            "UXTW x0, w0, x0 must Err; extra operand is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_uxtw_regression_wd() {
        let ops = [Operand::Reg("w0".into()), Operand::Reg("w0".into())];
        assert!(
            encode_uxtw(&ops).is_err(),
            "UXTW w0, w0 must Err; UXTW dest is Xd only (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_uxtw_regression_sp() {
        let ops = [Operand::Reg("wsp".into()), Operand::Reg("w0".into())];
        assert!(
            encode_uxtw(&ops).is_err(),
            "UXTW wsp, w0 must Err; register 31 is WZR/XZR not SP/WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_uxtw_regression_fp() {
        let ops = [Operand::Reg("d0".into()), Operand::Reg("w1".into())];
        assert!(
            encode_uxtw(&ops).is_err(),
            "UXTW d0, w1 must Err; FP/SIMD registers are not UXTW operands (llvm-mc rejects it)"
        );
    }
}
