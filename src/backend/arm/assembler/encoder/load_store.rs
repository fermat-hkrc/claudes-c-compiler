...
#[cfg(test)]
mod encode_ldaxr_stlxr_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs:1-7 32-bit AArch64 words; encoder/mod.rs:354-359 ldaxr/stlxr/ldaxrb/stlxrb/ldaxrh/stlxrh dispatch;
    //   ARM ARM Load/Store Exclusive: size 001000 0 L 0 Rs o0 Rt2 Rn Rt;
    //   Rt is Wt/Xt (31=ZR); Rn is Xn|SP; Ws is Wt (31=WZR); offset {,#0}; o0=1; Rt2=11111.
    // Stronger considered:
    //   - State machine: rejected — encode_ldaxr_stlxr is a pure function with no lifecycle
    //   - Algebraic round-trip via in-tree decoder: rejected — no exclusive-acquire decoder
    //   - encode_ldxr_stxr / encode_ldxp_stxp / encode_ldar_stlr as differential siblings:
    //     rejected — relaxed exclusive / exclusive-pair / ordered non-exclusive, different job
    // Weaker available: algebraic.invariant (ARM field unpack), algebraic.metamorphic (L / size / o0),
    //   negative_error (arity / extra / SP / FP / W-base / XZR-base / X-Ws / offset / Ws-overlap)
    // Differential: candidate=encode_ldaxr_stlxr, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=load (Reg(Rt), Mem{Rn, 0}, is_load=true, forced_size)
    //           <-> `{ldaxr|ldaxrb|ldaxrh} Rt, [Rn]`;
    //           store (Reg(Ws), Reg(Rt), Mem{Rn, 0}, is_load=false, forced_size)
    //           <-> `{stlxr|stlxrb|stlxrh} Ws, Rt, [Rn]`
    ...
    /// llvm-mc / ARM: STLXR status must not also be a source (Rt or Xn).
    /// WZR vs SP (both encode as 31) is allowed: `stlxr wzr, x0, [sp]`.
    fn stlxr_ws_aliases_source(ws: u32, rt: u32, rn: u32) -> bool {
        ws == rt || (rn != 31 && ws == rn)
    }
    ...
    fn sut_word(ops: &[Operand], is_load: bool, forced: Option<u32>) -> Result<u32, String> {
        match encode_ldaxr_stlxr(ops, is_load, forced)? {
            EncodeResult::Word(w) => Ok(w),
            other => Err(format!("expected Word, got {:?}", other)),
        }
    }
    ...
    /// Unpack exclusive-single fields per ARM ARM (not a copy of the SUT packer).
    fn unpack_ldaxr_stlxr(word: u32) -> (u32, u32, u32, u32, u32, u32, u32, u32) {
        let size = (word >> 30) & 0b11;
        let l = (word >> 22) & 1;
        let o1 = (word >> 21) & 1;
        let rs = (word >> 16) & 0x1f;
    ...
    fn fixed_ldaxr_stlxr_bits(word: u32) -> bool {
        ((word >> 24) & 0x3f) == 0b001000
            && ((word >> 23) & 1) == 0
            && ((word >> 21) & 1) == 0
            && ((word >> 15) & 1) == 1
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_ldaxr_x0_x1() {
        let want = 0xc85ffc20u32;
        let mc = llvm_mc_word("ldaxr x0, [x1]").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = load_ops(0, 1, true);
        let sut = sut_word(&ops, true, None).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_ldaxr_w0_x1() {
        let want = 0x885ffc20u32;
        let mc = llvm_mc_word("ldaxr w0, [x1]").expect("llvm-mc KAT w");
        assert_eq!(mc, want, "llvm-mc KAT w mapping broken");
        let ops = load_ops(0, 1, false);
        let sut = sut_word(&ops, true, None).expect("SUT KAT w");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_ldaxrb_w0_x1() {
        let want = 0x085ffc20u32;
        let mc = llvm_mc_word("ldaxrb w0, [x1]").expect("llvm-mc KAT b");
        assert_eq!(mc, want, "llvm-mc KAT b mapping broken");
        let ops = load_ops(0, 1, false);
        let sut = sut_word(&ops, true, Some(0b00)).expect("SUT KAT b");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_ldaxrh_w0_x1() {
        let want = 0x485ffc20u32;
        let mc = llvm_mc_word("ldaxrh w0, [x1]").expect("llvm-mc KAT h");
        assert_eq!(mc, want, "llvm-mc KAT h mapping broken");
        let ops = load_ops(0, 1, false);
        let sut = sut_word(&ops, true, Some(0b01)).expect("SUT KAT h");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_stlxr_w0_x1_x2() {
        let want = 0xc800fc41u32;
        let mc = llvm_mc_word("stlxr w0, x1, [x2]").expect("llvm-mc KAT stlxr");
        assert_eq!(mc, want, "llvm-mc KAT stlxr mapping broken");
        let ops = store_ops(0, 1, 2, true);
        let sut = sut_word(&ops, false, None).expect("SUT KAT stlxr");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_stlxr_w0_w1_x2() {
        let want = 0x8800fc41u32;
        let mc = llvm_mc_word("stlxr w0, w1, [x2]").expect("llvm-mc KAT stlxr w");
        assert_eq!(mc, want, "llvm-mc KAT stlxr w mapping broken");
        let ops = store_ops(0, 1, 2, false);
        let sut = sut_word(&ops, false, None).expect("SUT KAT stlxr w");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_ldaxr_sp() {
        let want = 0xc85fffe0u32;
        let mc = llvm_mc_word("ldaxr x0, [sp]").expect("llvm-mc KAT sp");
        assert_eq!(mc, want, "llvm-mc KAT sp mapping broken");
        let ops = load_ops(0, 31, true);
        let sut = sut_word(&ops, true, None).expect("SUT KAT sp");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_ldaxr_stlxr_kat_llvm_mc_ldaxr_lr() {
        let want = 0xc85ffc5eu32;
        let mc = llvm_mc_word("ldaxr lr, [x2]").expect("llvm-mc KAT lr");
        assert_eq!(mc, want, "llvm-mc KAT lr mapping broken");
        let ops = vec![
            Operand::Reg("lr".into()),
            Operand::Mem {
    ...
    #[test]
    fn test_encode_ldaxr_stlxr_regression_extra_operand() {
        let mut ops = store_ops(0, 1, 2, true);
        ops.push(Operand::Reg("x2".into()));
        assert!(
            encode_ldaxr_stlxr(&ops, false, None).is_err(),
            "stlxr w0, x1, [x2], x2 must Err; llvm-mc rejects a 4th operand"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_sp_as_rt() {
        let ops = load_ops_named("sp", "x0");
        assert!(
            encode_ldaxr_stlxr(&ops, true, None).is_err(),
            "ldaxr sp, [x0] must Err; llvm-mc rejects SP as Rt"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_w_base() {
        let ops = load_ops_named("x0", "w1");
        assert!(
            encode_ldaxr_stlxr(&ops, true, None).is_err(),
            "ldaxr x0, [w1] must Err; llvm-mc rejects a W register as base"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_xzr_as_base() {
        let ops = load_ops_named("x0", "xzr");
        assert!(
            encode_ldaxr_stlxr(&ops, true, None).is_err(),
            "ldaxr x0, [xzr] must Err; llvm-mc rejects XZR as base"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_fp_as_rt() {
        let ops = load_ops_named("d0", "x1");
        assert!(
            encode_ldaxr_stlxr(&ops, true, None).is_err(),
            "ldaxr d0, [x1] must Err; llvm-mc rejects SIMD/FP as Rt"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_x_as_ws() {
        let ops = vec![
            Operand::Reg("x0".into()),
            Operand::Reg("x1".into()),
            Operand::Mem {
        ...
        assert!(
            encode_ldaxr_stlxr(&ops, false, None).is_err(),
            "stlxr x0, x1, [x2] must Err; llvm-mc rejects X as STLXR status"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_x_data_byte() {
        let ops = load_ops_named("x0", "x1");
        assert!(
            encode_ldaxr_stlxr(&ops, true, Some(0b00)).is_err(),
            "ldaxrb x0, [x1] must Err; llvm-mc requires Wt for byte exclusive"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_nonzero_offset() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert!(
            encode_ldaxr_stlxr(&ops, true, None).is_err(),
            "ldaxr x0, [x1, #-1] must Err; llvm-mc: index must be absent or #0"
        );
    }

    #[test]
    fn test_encode_ldaxr_stlxr_regression_ws_overlap() {
        let ops = store_ops(0, 0, 1, false);
        assert!(
            encode_ldaxr_stlxr(&ops, false, None).is_err(),
            "stlxr w0, w0, [x1] must Err; llvm-mc: unpredictable STXR, status is also a source"
        );
    }
    ...
    proptest! {
        ...
        // Oracle: differential
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_diff_llvm_mc(
            rt in reg_edge(),
            rn in reg_edge(),
            ws in reg_edge(),
            is_load in any::<bool>(),
            ...
            let is_64 = data_is_64(variant, is_64);
            if !is_load {
                prop_assume!(!stlxr_ws_aliases_source(ws, rt, rn));
            }
            let ops = valid_ops(rt, rn, ws, is_load, is_64);
            let mnem = mnemonic(is_load, variant);
            let asm = if is_load {
        ...
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_arm_fields(
            rt in reg_edge(),
            rn in reg_edge(),
            ws in reg_edge(),
            is_load in any::<bool>(),
            ...
            let word = sut_word(&ops, is_load, forced_size(variant))
                .unwrap_or_else(|e| panic!("SUT rejected valid exclusive: {}", e));
            prop_assert!(
                fixed_ldaxr_stlxr_bits(word),
                "fixed exclusive-acquire bits wrong: {:#010x}",
                word
            );
            let (size, l, o1, rs, o0, rt2, got_rn, got_rt) = unpack_ldaxr_stlxr(word);
            prop_assert_eq!(size, expected_size(variant, is_64), "size");
            prop_assert_eq!(l, if is_load { 1 } else { 0 }, "L");
            prop_assert_eq!(o1, 0, "o1 must be 0 (not pair)");
            prop_assert_eq!(rs, if is_load { 31 } else { ws }, "Rs");
            prop_assert_eq!(o0, 1, "o0 must be 1 (acquire/release)");
            prop_assert_eq!(rt2, 0b11111, "Rt2");
            prop_assert_eq!(got_rn, rn, "Rn");
            prop_assert_eq!(got_rt, rt, "Rt");
        }
        ...
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_metamorphic_l_size_o0(
            rt in 0u32..=30,
            rn in reg_edge(),
            ws in reg_edge(),
            is_load in any::<bool>(),
            ...
            prop_assert_eq!(
                load ^ store,
                1u32 << 22,
                "L bit: ldaxr XOR stlxr(wzr) must be 1<<22; load={:#010x} store={:#010x}",
                load,
                store
            );
            ...
            if !stlxr_ws_aliases_source(ws, rt, rn) {
                let b = sut_word(&store_ops(ws, rt, rn, false), false, Some(0b00))
                    .unwrap_or_else(|e| panic!("byte: {}", e));
                let h = sut_word(&store_ops(ws, rt, rn, false), false, Some(0b01))
                    .unwrap_or_else(|e| panic!("half: {}", e));
            ...
            let ops = valid_ops(rt, rn, ws, is_load, is_64);
            let acq = sut_word(&ops, is_load, forced_size(variant))
                .unwrap_or_else(|e| panic!("ldaxr/stlxr: {}", e));
            let rel = match encode_ldxr_stxr(&ops, is_load, forced_size(variant)) {
                Ok(EncodeResult::Word(w)) => w,
                other => panic!("ldxr/stxr: {:?}", other),
            };
            prop_assert_eq!(
                acq ^ rel,
                1u32 << 15,
                "o0: encode_ldaxr_stlxr XOR encode_ldxr_stxr must be 1<<15; acq={:#010x} rel={:#010x}",
                acq,
                rel
            );
        }
        ...
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_neg_extra_operand(
            rt in reg_edge(),
            rn in reg_edge(),
            ws in reg_edge(),
            is_load in any::<bool>(),
            ...
            ops.push(extra);
            prop_assert!(
                encode_ldaxr_stlxr(&ops, is_load, forced_size(variant)).is_err(),
                "extra operand must Err (llvm-mc rejects a trailing operand); got {:?}",
                encode_ldaxr_stlxr(&ops, is_load, forced_size(variant))
            );
        }
        ...
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_neg_invalid_regs(
            kind in 0u32..=5,
            n in reg_edge(),
            other in reg_edge(),
            fp_kind in 0u32..=5,
            ...
            let (ops, is_load, forced, why) = match kind {
                0 => {
                    (
                        ...
                        true,
                        None,
                        "ldaxr sp, [Xn] must Err; llvm-mc rejects SP as Rt",
                    )
                }
                1 => {
                    let base = if n == 31 {
                    ...
                    (
                        ...
                        true,
                        None,
                        "ldaxr Xt, [Wn] must Err; llvm-mc rejects a W register as base",
                    )
                }
                2 => {
                    (
                        ...
                        true,
                        None,
                        "ldaxr Xt, [xzr] must Err; llvm-mc rejects XZR as base",
                    )
                }
                3 => {
                    (
                        ...
                        true,
                        None,
                        "ldaxr <FP>, [Xn] must Err; llvm-mc rejects SIMD/FP as Rt",
                    )
                }
                4 => {
                    (
                        ...
                        false,
                        None,
                        "stlxr Xt, Xt, [Xn] must Err; llvm-mc rejects X as STLXR status",
                    )
                }
                _ => {
                    (
                        ...
                        true,
                        Some(0b00),
                        "ldaxrb Xt, [Xn] must Err; llvm-mc requires Wt for byte/half",
                    )
                }
            };
            prop_assert!(
                encode_ldaxr_stlxr(&ops, is_load, forced).is_err(),
                "{}; got {:?}",
                why,
                encode_ldaxr_stlxr(&ops, is_load, forced)
            );
        }
        ...
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_neg_arity_shape(
            is_load in any::<bool>(),
            shape in 0u32..=8,
            rt in reg_edge(),
            offset in nonzero_offset(),
            ...
            prop_assert!(
                encode_ldaxr_stlxr(&ops, is_load, None).is_err(),
                "too-few / non-Mem / pre-post / invalid base / nonzero offset must Err (shape {}); got {:?}",
                shape,
                encode_ldaxr_stlxr(&ops, is_load, None)
            );
        }
        ...
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_neg_ws_overlap(
            rt in reg_edge(),
            rn in reg_edge(),
            variant in 0u32..=2,
            is_64 in any::<bool>(),
            ...
            let ops = store_ops(ws, rt, rn, is_64);
            prop_assert!(
                encode_ldaxr_stlxr(&ops, false, forced_size(variant)).is_err(),
                "stlxr with Ws aliasing Rt/Xn must Err (llvm-mc: status is also a source); got {:?}",
                encode_ldaxr_stlxr(&ops, false, forced_size(variant))
            );
        }
        ...
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_diff_alt_spellings(
            rt in reg_edge(),
            rn in reg_edge(),
            spelling in 0u32..=3,
        ) {
            let (rt_s, rn_s, asm_rt, asm_rn) = match spelling {
                0 => {
                    // x31 / xN as numeric 31 aliases xzr; llvm-mc canonicalizes
                    let rt_s = if rt == 31 { "x31".to_string() } else { format!("x{}", rt) };
                    let rn_s = if rn == 31 { "sp".to_string() } else { format!("x{}", rn) };
                    let asm_rt = if rt == 31 { "x31".to_string() } else { format!("x{}", rt) };
                    let asm_rn = rn_s.clone();
                    (rt_s, rn_s, asm_rt, asm_rn)
                }
                1 => {
                ...
                2 => {
                    let rt_s = if rt == 30 { "lr".to_string() } else if rt == 31 { "xzr".to_string() } else { format!("x{}", rt) };
                    let rn_s = if rn == 31 { "sp".to_string() } else { format!("x{}", rn) };
                    (rt_s.clone(), rn_s.clone(), rt_s, rn_s)
                }
                _ => {
                    let rt_s = if rt == 31 { "wzr".to_string() } else { format!("w{}", rt) };
                    let rn_s = if rn == 31 { "sp".to_string() } else { format!("x{}", rn) };
                    (rt_s.clone(), rn_s.clone(), rt_s, rn_s)
                }
            };
            ...
            let is_64 = spelling != 3;
            let asm = format!("ldaxr {}, [{}]", asm_rt, asm_rn);
            let sut = sut_word(&ops, true, None)
                .unwrap_or_else(|e| panic!("SUT rejected valid {}: {}", asm, e));
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid {}: {}", asm, e));
            prop_assert_eq!(sut, mc, "SUT vs llvm-mc mismatch for {}", asm);
        ...
        // Oracle: negative_error (coverage-gaps sweep: register-offset / expr mem)
        // Target: encoder.load_store.encode_ldaxr_stlxr
        #[test]
        fn encode_ldaxr_stlxr_neg_mem_index(
            rt in reg_edge(),
            rn in reg_edge(),
            idx in 0u32..=30,
            kind in 0u32..=2,
            ...
            let ops = vec![Operand::Reg(data_name(rt, true)), mem];
            prop_assert!(
                encode_ldaxr_stlxr(&ops, true, None).is_err(),
                "ldaxr with [Xn, Xm] / MemExpr must Err (llvm-mc: index must be absent or #0); got {:?}",
                encode_ldaxr_stlxr(&ops, true, None)
            );
        }
    }
}
...
#[cfg(test)]
mod encode_ldtr_sized_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs:1-7 32-bit AArch64 words; encoder/mod.rs:340-343 ldtrh/sttrh/ldtrb/sttrb dispatch;
    //   ARM ARM LDTRB/LDTRH/STTRB/STTRH: size 111 V=0 00 opc 0 imm9 10 Rn Rt;
    //   Rt is Wt (31=WZR), never Xt/SP/SIMD; Rn is Xn|SP; simm9 in [-256, 255].
    // Stronger considered:
    //   - State machine: rejected — encode_ldtr_sized is a pure function with no lifecycle
    //   - Algebraic round-trip via in-tree decoder: rejected — no LDTRB decoder
    //   - encode_ldur_stur / encode_ldr_str as differential siblings: rejected — different job
    // Weaker available: algebraic.invariant (ARM field unpack), algebraic.metamorphic (size / opc / Rt / Rn),
    //   negative_error (arity / extra / Xt dest / SP / W-base / XZR-base / SIMD / range / form)
    // Differential: candidate=encode_ldtr_sized, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=(Reg(Wt), Mem{Rn, imm9}, is_load, size) <-> `{ldtrb|ldtrh|sttrb|sttrh} Wt, [Rn{, #imm}]`

    use super::encode_ldtr_sized;
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    fn mnemonic(is_load: bool, size: u32) -> &'static str {
        match (is_load, size) {
            (true, 0) => "ldtrb",
            (true, _) => "ldtrh",
            (false, 0) => "sttrb",
    ...
    fn valid_ops(rt: u32, rn: u32, offset: i64) -> Vec<Operand> {
        vec![
            Operand::Reg(wt(rt)),
            Operand::Mem {
                base: rn_name(rn),
    ...
    fn sut_word(ops: &[Operand], is_load: bool, size: u32) -> Result<u32, String> {
        match encode_ldtr_sized(ops, is_load, size)? {
            EncodeResult::Word(w) => Ok(w),
            other => Err(format!("expected Word, got {:?}", other)),
        }
    }

    /// Unpack LDTRB/H/STTRB/H fields per ARM ARM (not a copy of the SUT packer).
    fn unpack_ldtr(word: u32) -> (u32, u32, u32, i64, u32, u32, u32) {
        let size = (word >> 30) & 0b11;
        let v = (word >> 26) & 1;
        let opc = (word >> 22) & 0b11;
        let imm9_raw = (word >> 12) & 0x1ff;
    ...
    fn fixed_ldtr_bits(word: u32) -> bool {
        ((word >> 27) & 0b111) == 0b111
            && ((word >> 24) & 0b11) == 0b00
            && ((word >> 21) & 1) == 0
    }
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_ldtr_sized_kat_llvm_mc_ldtrb_w0_x1() {
        let want = 0x38400820u32;
        let mc = llvm_mc_word("ldtrb w0, [x1]").expect("llvm-mc KAT ldtrb");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = valid_ops(0, 1, 0);
        let sut = sut_word(&ops, true, 0b00).expect("SUT KAT ldtrb");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldtr_sized_kat_llvm_mc_ldtrh_w0_x1() {
        let want = 0x78400820u32;
        let mc = llvm_mc_word("ldtrh w0, [x1]").expect("llvm-mc KAT ldtrh");
        assert_eq!(mc, want, "llvm-mc KAT ldtrh mapping broken");
        let ops = valid_ops(0, 1, 0);
        let sut = sut_word(&ops, true, 0b01).expect("SUT KAT ldtrh");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldtr_sized_kat_llvm_mc_sttrb_w0_x1() {
        let want = 0x38000820u32;
        let mc = llvm_mc_word("sttrb w0, [x1]").expect("llvm-mc KAT sttrb");
        assert_eq!(mc, want, "llvm-mc KAT sttrb mapping broken");
        let ops = valid_ops(0, 1, 0);
        let sut = sut_word(&ops, false, 0b00).expect("SUT KAT sttrb");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldtr_sized_kat_llvm_mc_sttrh_w0_x1() {
        let want = 0x78000820u32;
        let mc = llvm_mc_word("sttrh w0, [x1]").expect("llvm-mc KAT sttrh");
        assert_eq!(mc, want, "llvm-mc KAT sttrh mapping broken");
        let ops = valid_ops(0, 1, 0);
        let sut = sut_word(&ops, false, 0b01).expect("SUT KAT sttrh");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldtr_sized_kat_llvm_mc_ldtrb_imm_bounds() {
        let lo = llvm_mc_word("ldtrb w0, [x1, #-256]").expect("llvm-mc KAT #-256");
        assert_eq!(lo, 0x38500820u32);
        let hi = llvm_mc_word("ldtrb w0, [sp, #255]").expect("llvm-mc KAT #255");
        assert_eq!(hi, 0x384ffbe0u32);
        assert_eq!(sut_word(&valid_ops(0, 1, -256), true, 0).unwrap(), lo);
        assert_eq!(sut_word(&valid_ops(0, 31, 255), true, 0).unwrap(), hi);
    }

    #[test]
    fn test_encode_ldtr_sized_regression_extra_operand() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Mem {
                base: "x0".into(),
        ...
        assert!(
            encode_ldtr_sized(&ops, false, 0).is_err(),
            "sttrb w0, [x0, #-256], x2 must Err; llvm-mc rejects a 3rd operand"
        );
    }

    #[test]
    fn test_encode_ldtr_sized_regression_xt_dest() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "x0".into(),
        ...
        assert!(
            encode_ldtr_sized(&ops, false, 0).is_err(),
            "sttrb x0, [x0, #-256] must Err; ARM ARM dest is Wt; llvm-mc rejects Xt"
        );
    }

    #[test]
    fn test_encode_ldtr_sized_regression_sp_as_rt() {
        let ops = [
            Operand::Reg("sp".into()),
            Operand::Mem {
                base: "x0".into(),
        ...
        assert!(
            encode_ldtr_sized(&ops, false, 0).is_err(),
            "sttrb sp, [x0, #-256] must Err; llvm-mc rejects SP as Rt"
        );
    }

    #[test]
    fn test_encode_ldtr_sized_regression_fp_dest() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Mem {
                base: "x0".into(),
        ...
        assert!(
            encode_ldtr_sized(&ops, true, 0).is_err(),
            "ldtrb d0, [x0] must Err; llvm-mc rejects SIMD Rt on LDTRB"
        );
    }

    #[test]
    fn test_encode_ldtr_sized_regression_w_base() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Mem {
                base: "w0".into(),
        ...
        assert!(
            encode_ldtr_sized(&ops, true, 0).is_err(),
            "ldtrb w0, [w0] must Err; llvm-mc rejects a W register as base"
        );
    }

    #[test]
    fn test_encode_ldtr_sized_regression_xzr_base() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Mem {
                base: "xzr".into(),
        ...
        assert!(
            encode_ldtr_sized(&ops, true, 0).is_err(),
            "ldtrb w0, [xzr] must Err; llvm-mc rejects XZR as base (Rn=31 is SP)"
        );
    }

    #[test]
    fn test_encode_ldtr_sized_regression_imm9_range() {
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Mem {
                base: "x0".into(),
        ...
        assert!(
            encode_ldtr_sized(&ops, false, 0).is_err(),
            "sttrb w0, [x0, #-257] must Err; llvm-mc requires simm9 in [-256, 255]"
        );
    }
    ...
    proptest! {
        ...
        // Oracle: differential
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_diff_llvm_mc(
            rt in reg_edge(),
            rn in reg_edge(),
            simm in imm9_in_range(),
            is_load in any::<bool>(),
            size in 0u32..=1u32,
            upper in any::<bool>(),
        ) {
            let mut rt_n = wt(rt);
            let mut rn_n = rn_name(rn);
            if upper {
                rt_n = rt_n.to_uppercase();
                rn_n = rn_n.to_uppercase();
            }
            let mn = mnemonic(is_load, size);
            let asm = format!("{} {}, {}", mn, rt_n, asm_mem(&rn_n, simm));
            let ops = vec![
                Operand::Reg(rt_n.clone()),
                Operand::Mem {
                    base: rn_n.clone(),
            ...
            let sut = sut_word(&ops, is_load, size)
                .unwrap_or_else(|e| panic!("SUT rejected valid {}: {}", asm, e));
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid {}: {}", asm, e));
            prop_assert_eq!(sut, mc, "SUT vs llvm-mc mismatch for {}", asm);
        ...
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_arm_fields(
            rt in reg_edge(),
            rn in reg_edge(),
            simm in imm9_in_range(),
            is_load in any::<bool>(),
            ...
            let ops = valid_ops(rt, rn, simm);
            let word = sut_word(&ops, is_load, size)
                .unwrap_or_else(|e| panic!("SUT rejected in-range LDTR*: {}", e));
            let (got_size, v, opc, got_imm, op2, got_rn, got_rt) = unpack_ldtr(word);
            prop_assert!(fixed_ldtr_bits(word), "fixed LDTR bits, word={:#010x}", word);
            prop_assert_eq!(got_size, size, "size field word={:#010x}", word);
            prop_assert_eq!(v, 0, "V must be 0 word={:#010x}", word);
            prop_assert_eq!(opc, if is_load { 0b01 } else { 0b00 }, "opc word={:#010x}", word);
            prop_assert_eq!(got_imm, simm, "imm9 word={:#010x}", word);
            prop_assert_eq!(op2, 0b10, "bits[11:10] must be 10 word={:#010x}", word);
            prop_assert_eq!(got_rn, rn, "Rn word={:#010x}", word);
            prop_assert_eq!(got_rt, rt, "Rt word={:#010x}", word);
        }

        // Oracle: algebraic.metamorphic
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_metamorphic_fields(
            rt in 0u32..=30,
            rn in 0u32..=30,
            simm in IMM_MIN..=(IMM_MAX - 1),
            is_load in any::<bool>(),
        ) {
            let e = |rt: u32, rn: u32, simm: i64, load: bool, sz: u32| {
                sut_word(&valid_ops(rt, rn, simm), load, sz).unwrap()
            };
            let w0 = e(rt, rn, simm, is_load, 0);
            let w_sz = e(rt, rn, simm, is_load, 1);
            let w_st = e(rt, rn, simm, !is_load, 0);
            let w_rt = e(rt + 1, rn, simm, is_load, 0);
            let w_rn = e(rt, rn + 1, simm, is_load, 0);
            let w_imm = e(rt, rn, simm + 1, is_load, 0);
            prop_assert_eq!(w_sz ^ w0, 1u32 << 30, "size bit 30");
            prop_assert_eq!(w0 ^ w_st, 1u32 << 22, "opc bit 22");
            prop_assert_eq!(w_rt.wrapping_sub(w0), 1u32, "Rt +1");
            prop_assert_eq!(w_rn.wrapping_sub(w0), 32u32, "Rn +1");
            let imm0 = (w0 >> 12) & 0x1ff;
            let imm1 = (w_imm >> 12) & 0x1ff;
            prop_assert_eq!(imm1, (imm0 + 1) & 0x1ff, "imm9 +1");
            prop_assert_eq!(w_imm & !0x1ff000, w0 & !0x1ff000, "only imm9 field changes");
        }

        // Oracle: negative_error
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_neg_arity(
            is_load in any::<bool>(),
            size in 0u32..=1u32,
            kind in 0u32..=3u32,
            rt in reg_edge(),
            ...
            prop_assert!(
                encode_ldtr_sized(&ops, is_load, size).is_err(),
        ...
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_neg_extra_operand(
            rt in reg_edge(),
            rn in reg_edge(),
            simm in imm9_in_range(),
            is_load in any::<bool>(),
            ...
            prop_assert!(
                encode_ldtr_sized(&ops, is_load, size).is_err(),
                "third operand must Err; llvm-mc rejects extra operands on ldtrb/ldtrh"
            );
        }
        ...
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_neg_xt_dest(
            rt in reg_edge(),
            rn in reg_edge(),
            simm in imm9_in_range(),
            is_load in any::<bool>(),
            ...
            prop_assert!(
                encode_ldtr_sized(&ops, is_load, size).is_err(),
                "Xt/lr dest must Err; ARM ARM LDTRB/H dest is Wt; llvm-mc rejects ldtrb x0"
            );
        }
        ...
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_neg_invalid_regs(
            kind in 0u32..=4u32,
            n in 0u32..=31u32,
            simm in imm9_in_range(),
            is_load in any::<bool>(),
            ...
            let ops = match kind {
                ...
                2 => vec![
                    Operand::Reg(format!("{}{}", fp, n.min(31))),
                    Operand::Mem {
                        base: rn_name(n),
                        offset: simm,
                    },
                ...
                3 => vec![
                    Operand::Reg(wt(n)),
                    Operand::Mem {
                        base: format!("w{}", if n == 31 { 0 } else { n }),
                        offset: simm,
                    },
                ],
                _ => vec![
            ...
            prop_assert!(
                encode_ldtr_sized(&ops, is_load, size).is_err(),
                "invalid Rt/Rn must Err; llvm-mc rejects SP/WSP/SIMD Rt, W base, XZR base kind={}",
                kind
            );
        }
        ...
        // Target: encoder.load_store.encode_ldtr_sized
        #[test]
        fn encode_ldtr_sized_neg_offset_and_form(
            rt in reg_edge(),
            rn in reg_edge(),
            is_load in any::<bool>(),
            size in 0u32..=1u32,
            use_offset in any::<bool>(),
            bad_offset in imm9_out_of_range(),
            form in 0u32..=5u32,
        ) {
            let base = rn_name(rn);
            let ops = if use_offset {
                ...
                let addr = match form {
                    0 => Operand::MemPreIndex {
                        base,
                        offset: 0,
                    },
                    1 => Operand::MemPostIndex {
                        base,
                        offset: 0,
                    },
                    2 => Operand::MemRegOffset {
                    ...
                    3 => Operand::Imm(0),
                    4 => Operand::Symbol("foo".into()),
                    _ => Operand::Label("l".into()),
                };
                vec![Operand::Reg(wt(rt)), addr]
            };
            prop_assert!(
                encode_ldtr_sized(&ops, is_load, size).is_err(),
                "out-of-range simm9 or non-Mem form must Err; llvm-mc range [-256,255], unscaled only"
            );
        }

        // Oracle: negative_error
        // Target: encoder.load_store.encode_ldtr_sized
        // Sweep: documented unscaled-only form (pre/post/regoff/Imm/Symbol/Label) must Err,
        // independently of the failing imm9-wrap case.
        #[test]
        fn encode_ldtr_sized_neg_bad_form(
            rt in reg_edge(),
            rn in reg_edge(),
            is_load in any::<bool>(),
            size in 0u32..=1u32,
            ...
            let ops = vec![Operand::Reg(wt(rt)), addr];
            prop_assert!(
                encode_ldtr_sized(&ops, is_load, size).is_err(),
                "non-Mem addressing must Err; ARM ARM LDTRB is unscaled [Xn{{, #simm}}] only"
            );
        }

        // Oracle: differential
        // Target: encoder.load_store.encode_ldtr_sized
        // Sweep: w31 is an alias of wzr (llvm-mc / ARM ARM register 31 = WZR).
        #[test]
        fn encode_ldtr_sized_diff_w31_alias(
            rn in reg_edge(),
            simm in imm9_in_range(),
            is_load in any::<bool>(),
            size in 0u32..=1u32,
            ...
            let rn_n = rn_name(rn);
            let mn = mnemonic(is_load, size);
            let mem = asm_mem(&rn_n, simm);
            let asm_w31 = format!("{} w31, {}", mn, mem);
            let asm_wzr = format!("{} wzr, {}", mn, mem);
            let ops_w31 = vec![
            ...
            let sut_w31 = sut_word(&ops_w31, is_load, size)
                .unwrap_or_else(|e| panic!("SUT rejected w31: {}", e));
            let sut_wzr = sut_word(&ops_wzr, is_load, size)
                .unwrap_or_else(|e| panic!("SUT rejected wzr: {}", e));
            let mc = llvm_mc_word(&asm_wzr)
                .unwrap_or_else(|e| panic!("llvm-mc rejected {}: {}", asm_wzr, e));
            prop_assert_eq!(sut_w31, sut_wzr, "w31 must encode as wzr ({})", asm_w31);
            prop_assert_eq!(sut_wzr, mc, "wzr must match llvm-mc {}", asm_wzr);
        }
    }
}
...
mod encode_ldrs_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs:1-7 32-bit AArch64 words; encoder/mod.rs:334-335
    //   "ldrsb" => encode_ldrs(operands, 0b00); "ldrsh" => encode_ldrs(operands, 0b01);
    //   README.md:221 Loads/Stores table lists ldrsb, ldrsh;
    //   ARM ARM LDRSB/LDRSH unsigned: size 111 V=0 01 opc imm12 Rn Rt
    //   (size=00 byte / 01 half; opc=10 Xt / 11 Wt; pimm=imm12*scale, scale=1 or 2);
    //   LDURSB/LDURSH: size 111 V=0 00 opc 0 imm9 00 Rn Rt (simm9 in [-256,255]);
    //   pre bits[11:10]=11, post bits[11:10]=01; register: bit21=1 option S bits[11:10]=10.
    //   Rt is Wt|Xt (31=WZR/XZR), never SP/SIMD; Rn is Xn|SP (not XZR).
    // Stronger considered:
    //   - State machine: rejected — encode_ldrs is a pure function with no lifecycle
    //   - Algebraic round-trip via in-tree decoder: rejected — no LDRSB/LDRSH decoder
    //   - encode_ldrsw / encode_ldr_str / encode_ldur_stur as differential siblings: rejected —
    //     different jobs (LDRSW word sign-extend to Xt, LDR/STR, generic unscaled)
    // Weaker available: algebraic.invariant (ARM field unpack), algebraic.metamorphic (Rt/Rn/imm12/opc/size),
    //   negative_error (arity / extra / SP / FP / W-base / XZR-base / offset range / W-index / writeback overlap)
    // Differential: candidate=encode_ldrs, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=(Reg(Wt|Xt), Mem{Xn|SP, imm}) <-> `ldrsb|ldrsh Wt|Xt, [Xn|SP{, #imm}]`

    use super::encode_ldrs;
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    fn mnemonic(size: u32) -> &'static str {
        if size == 0 { "ldrsb" } else { "ldrsh" }
    }

    fn scale(size: u32) -> i64 {
        1i64 << size
    }
    ...
    fn rn_name(n: u32) -> String {
        if n == 31 { "sp".into() } else { format!("x{}", n) }
    }
    ...
    /// Unpack LDRSB/LDRSH fields per ARM ARM (not a copy of the SUT packer).
    fn unpack_ldrs(word: u32) -> (u32, u32, u32, u32, u32, u32, u32, u32, u32) {
        let size = (word >> 30) & 0b11;
        let bits29_27 = (word >> 27) & 0b111;
        let v = (word >> 26) & 1;
        let bits25_24 = (word >> 24) & 0b11;
    ...
    fn is_unsigned_pimm(offset: i64, size: u32) -> bool {
        let sc = scale(size);
        offset >= 0 && offset % sc == 0 && (offset / sc) < 4096
    }
    ...
    fn offset_out_of_range(size: u32) -> impl Strategy<Value = i64> {
        if size == 0 {
            prop_oneof![
                Just(SIMM_MIN - 1),
                Just(-257i64),
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_ldrs_kat_llvm_mc_x0_x1() {
        let want = 0x39800020u32;
        let mc = llvm_mc_word("ldrsb x0, [x1]").expect("llvm-mc KAT ldrsb");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
        ...
        let sut = sut_word(&ops, 0).expect("SUT KAT ldrsb");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldrs_kat_llvm_mc_w0_ldrsh() {
        let want = 0x79c00020u32;
        let mc = llvm_mc_word("ldrsh w0, [x1]").expect("llvm-mc KAT ldrsh w");
        assert_eq!(mc, want, "llvm-mc KAT ldrsh w mapping broken");
        let ops = [
            Operand::Reg("w0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        let sut = sut_word(&ops, 1).expect("SUT KAT ldrsh w");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldrs_kat_llvm_mc_pimm_max() {
        let want_b = 0x39bffc20u32;
        let mc_b = llvm_mc_word("ldrsb x0, [x1, #4095]").expect("llvm-mc KAT pimm byte max");
        assert_eq!(mc_b, want_b, "llvm-mc KAT pimm byte max mapping broken");
        let ops_b = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert_eq!(sut_word(&ops_b, 0).expect("SUT KAT pimm byte max"), want_b);

        let want_h = 0x79bffc20u32;
        let mc_h = llvm_mc_word("ldrsh x0, [x1, #8190]").expect("llvm-mc KAT pimm half max");
        assert_eq!(mc_h, want_h, "llvm-mc KAT pimm half max mapping broken");
        let ops_h = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert_eq!(sut_word(&ops_h, 1).expect("SUT KAT pimm half max"), want_h);
    }

    #[test]
    fn encode_ldrs_kat_llvm_mc_unscaled_neg() {
        let want = 0x389ff020u32;
        let mc = llvm_mc_word("ldrsb x0, [x1, #-1]").expect("llvm-mc KAT #-1");
        assert_eq!(mc, want, "llvm-mc KAT #-1 mapping broken");
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
        ...
        let sut = sut_word(&ops, 0).expect("SUT KAT #-1");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_ldrs_kat_llvm_mc_pre_post_regoff() {
        let pre = llvm_mc_word("ldrsb x0, [x1, #4]!").expect("llvm-mc pre");
        assert_eq!(pre, 0x38804c20u32);
        let post = llvm_mc_word("ldrsb w0, [x1], #4").expect("llvm-mc post");
        assert_eq!(post, 0x38c04420u32);
        let reg = llvm_mc_word("ldrsh x0, [x1, x2]").expect("llvm-mc reg");
        assert_eq!(reg, 0x78a26820u32);
        let sp = llvm_mc_word("ldrsb x0, [sp, #1]").expect("llvm-mc sp");
        assert_eq!(sp, 0x398007e0u32);
        let xzr = llvm_mc_word("ldrsb xzr, [x1]").expect("llvm-mc xzr");
        assert_eq!(xzr, 0x3980003fu32);
        let wzr = llvm_mc_word("ldrsb wzr, [x1]").expect("llvm-mc wzr");
        assert_eq!(wzr, 0x39c0003fu32);
        let lr = llvm_mc_word("ldrsb x30, [x2]").expect("llvm-mc x30");
        assert_eq!(lr, 0x3980005eu32);
        let sxtw = llvm_mc_word("ldrsh w0, [x1, w2, sxtw]").expect("llvm-mc sxtw");
        assert_eq!(sxtw, 0x78e2c820u32);
    }
    ...
    proptest! {
        ...
        // Oracle: differential
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_diff_unsigned_llvm_mc(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            rn in reg_edge(),
            ...
            let offset = (imm12 as i64) * scale(size);
            let rt_n = rt_name(rt, is_64);
            let rn_n = rn_name(rn);
            let asm = format!("{} {}, {}", mnemonic(size), rt_n, asm_mem(&rn_n, offset));
            let ops = [
                Operand::Reg(rt_n.clone()),
                Operand::Mem {
                    base: rn_n,
        ...
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_diff_unscaled_pre_post_llvm_mc(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            rn in reg_edge(),
            ...
            let rt_n = rt_name(rt, is_64);
            let rn_n = rn_name(rn);
            let mnem = mnemonic(size);
            let (ops, asm) = match form {
                0 => {
                    let asm = format!("{} {}, {}", mnem, rt_n, asm_mem(&rn_n, simm));
                    let ops = vec![
                        Operand::Reg(rt_n.clone()),
                        Operand::Mem {
                            base: rn_n.clone(),
                ...
                1 => {
                    let asm = format!("{} {}, [{}, #{}]!", mnem, rt_n, rn_n, simm);
                    let ops = vec![
                        Operand::Reg(rt_n.clone()),
                        Operand::MemPreIndex {
                            base: rn_n.clone(),
        ...
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_diff_regoff_llvm_mc(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            rn in reg_edge(),
            ...
            let amount: u8 = if size == 0 { 0 } else if amount_bit == 1 { 1 } else { 0 };
            let (extend, index, option, s_bit) = match ext_kind {
                0 => ("lsl", rm_x(rm), 0b011u32, if amount > 0 { 1u32 } else { 0 }),
                1 => ("sxtx", rm_x(rm), 0b111u32, if amount > 0 { 1u32 } else { 0 }),
                2 => ("uxtw", rm_w(rm), 0b010u32, if amount > 0 { 1u32 } else { 0 }),
                _ => ("sxtw", rm_w(rm), 0b110u32, if amount > 0 { 1u32 } else { 0 }),
            };
            let rt_n = rt_name(rt, is_64);
            let rn_n = rn_name(rn);
            let mnem = mnemonic(size);
            let asm = if amount == 0 {
                if extend == "lsl" {
                    format!("{} {}, [{}, {}]", mnem, rt_n, rn_n, index)
                } else {
                    format!("{} {}, [{}, {}, {}]", mnem, rt_n, rn_n, index, extend)
                }
            } else {
                format!("{} {}, [{}, {}, {} #{}]", mnem, rt_n, rn_n, index, extend, amount)
            };
            let ops = [
                Operand::Reg(rt_n.clone()),
                Operand::MemRegOffset {
        ...
        fn encode_ldrs_arm_fields(
            ...
            let opc_exp = if is_64 { 0b10u32 } else { 0b11 };
            let offset = (imm12 as i64) * scale(size);
            let wu = sut_word(&[
                Operand::Reg(rt_name(rt, is_64)),
                Operand::Mem {
                    base: rn_name(rn),
                    offset,
                },
            ], size)
            .expect("unsigned LDRSB/LDRSH");
            let (got_size, b2927, v, b2524, opc, bit21, b1110, got_rn, got_rt) = unpack_ldrs(wu);
            prop_assert_eq!(got_size, size, "unsigned size word={:#010x}", wu);
            prop_assert_eq!(b2927, 0b111, "unsigned [29:27] word={:#010x}", wu);
            prop_assert_eq!(v, 0, "unsigned V word={:#010x}", wu);
            prop_assert_eq!(b2524, 0b01, "unsigned [25:24] word={:#010x}", wu);
            prop_assert_eq!(opc, opc_exp, "unsigned opc word={:#010x}", wu);
            prop_assert_eq!(unpack_imm12(wu), imm12, "unsigned imm12 word={:#010x}", wu);
            prop_assert_eq!(got_rn, rn, "unsigned Rn word={:#010x}", wu);
            prop_assert_eq!(got_rt, rt, "unsigned Rt word={:#010x}", wu);
            let _ = (bit21, b1110);

            // Force the unscaled path: negative, or (halfword and odd).
            let simm_u = if size == 0 {
                if simm >= 0 { -1 - (simm % 256) } else { simm }
            } else if simm >= 0 && simm % 2 == 0 {
                if simm == 0 { 1 } else { simm - 1 }
            } else {
                simm
            };
            prop_assume!(simm_u >= SIMM_MIN && simm_u <= SIMM_MAX);
            prop_assume!(!is_unsigned_pimm(simm_u, size));
            ...
            .expect("unscaled LDRSB/LDRSH");
            let (got_size, b2927, v, b2524, opc, bit21, b1110, got_rn, got_rt) = unpack_ldrs(wn);
            prop_assert_eq!(got_size, size, "unscaled size word={:#010x}", wn);
            prop_assert_eq!(b2927, 0b111, "unscaled [29:27] word={:#010x}", wn);
            prop_assert_eq!(v, 0, "unscaled V word={:#010x}", wn);
            prop_assert_eq!(b2524, 0b00, "unscaled [25:24] word={:#010x}", wn);
            prop_assert_eq!(opc, opc_exp, "unscaled opc word={:#010x}", wn);
            prop_assert_eq!(bit21, 0, "unscaled bit21 word={:#010x}", wn);
            prop_assert_eq!(b1110, 0b00, "unscaled [11:10] word={:#010x}", wn);
            prop_assert_eq!(unpack_imm9(wn), simm_u, "unscaled imm9 word={:#010x}", wn);
            prop_assert_eq!(got_rn, rn, "unscaled Rn word={:#010x}", wn);
            prop_assert_eq!(got_rt, rt, "unscaled Rt word={:#010x}", wn);
            ...
            .expect("pre LDRSB/LDRSH");
            let (_, _, _, b2524, _, bit21, b1110, _, _) = unpack_ldrs(wp);
            prop_assert_eq!(b2524, 0b00, "pre [25:24] word={:#010x}", wp);
            prop_assert_eq!(bit21, 0, "pre bit21 word={:#010x}", wp);
            prop_assert_eq!(b1110, 0b11, "pre [11:10] word={:#010x}", wp);
            prop_assert_eq!(unpack_imm9(wp), simm, "pre imm9 word={:#010x}", wp);
            ...
            .expect("post LDRSB/LDRSH");
            let (_, _, _, _, _, _, b1110, _, _) = unpack_ldrs(wo);
            prop_assert_eq!(b1110, 0b01, "post [11:10] word={:#010x}", wo);
            prop_assert_eq!(unpack_imm9(wo), simm, "post imm9 word={:#010x}", wo);
            ...
            let wr = sut_word(&[
                ...
                Operand::MemRegOffset {
                    ...
                    index: rm_x(rm),
                    extend: Some("lsl".into()),
                    shift: Some(size as u8),
                },
            ], size)
            .expect("regoff LDRSB/LDRSH");
            let (got_size, b2927, v, b2524, opc, bit21, b1110, got_rn, got_rt) = unpack_ldrs(wr);
            prop_assert_eq!(got_size, size);
            prop_assert_eq!(b2927, 0b111);
            prop_assert_eq!(v, 0);
            prop_assert_eq!(b2524, 0b00);
            prop_assert_eq!(opc, opc_exp);
            prop_assert_eq!(bit21, 1, "regoff bit21 word={:#010x}", wr);
            prop_assert_eq!(b1110, 0b10, "regoff [11:10] word={:#010x}", wr);
            prop_assert_eq!((wr >> 16) & 0x1f, rm, "regoff Rm word={:#010x}", wr);
            prop_assert_eq!((wr >> 13) & 0b111, 0b011, "regoff option LSL");
            prop_assert_eq!((wr >> 12) & 1, if size == 0 { 0 } else { 1 }, "regoff S");
            prop_assert_eq!(got_rn, rn);
            prop_assert_eq!(got_rt, rt);
        }
        ...
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_metamorphic_rt_rn_imm(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in 0u32..=30,
            rn in 0u32..=30,
            ...
            let enc = |rt: u32, rn: u32, imm12: u32, size: u32, is_64: bool| {
                sut_word(&[
                    Operand::Reg(rt_name(rt, is_64)),
                    Operand::Mem {
                        base: rn_name(rn),
                        offset: (imm12 as i64) * scale(size),
                    },
                ], size)
                .expect("unsigned")
            };
            let w = enc(rt, rn, imm12, size, is_64);
            prop_assert_eq!(enc(rt + 1, rn, imm12, size, is_64).wrapping_sub(w), 1, "Rt+1 adds 1");
            prop_assert_eq!(enc(rt, rn + 1, imm12, size, is_64).wrapping_sub(w), 32, "Rn+1 adds 32");
            prop_assert_eq!(
                enc(rt, rn, imm12 + 1, size, is_64).wrapping_sub(w),
                1 << 10,
                "imm12+1 adds 1<<10"
            );
            ...
            let w64 = enc(rt, rn, imm12, size, true);
            let w32 = enc(rt, rn, imm12, size, false);
            prop_assert_eq!(w64 ^ w32, 1 << 22, "Xt vs Wt flips only opc bit 22");

            let wb = enc(rt, rn, imm12, 0, is_64);
            let wh = enc(rt, rn, imm12, 1, is_64);
            prop_assert_eq!(wb ^ wh, 1 << 30, "ldrsb vs ldrsh flips only size bit 30");
        }

        // Oracle: negative_error
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_neg_arity_kinds(
            size in size_bit(),
            rt in reg_edge(),
            is_64 in any::<bool>(),
            kind in bad_address_kind(),
            ...
            prop_assert!(
                encode_ldrs(&[], size).is_err(),
                "zero operands must Err"
            );
            let one = [Operand::Reg(rt_name(rt, is_64))];
            prop_assert!(
                encode_ldrs(&one, size).is_err(),
                "one operand must Err: {:?}",
                encode_ldrs(&one, size)
            );
            let ops = [Operand::Reg(rt_name(rt, is_64)), kind.clone()];
            prop_assert!(
                encode_ldrs(&ops, size).is_err(),
                "non-memory address operand must Err, got {:?} for {:?}",
                encode_ldrs(&ops, size),
                kind
            );
        }
        ...
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_neg_invalid_regs(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            rn in 0u32..=30,
            ...
            let sp_dest = [Operand::Reg("sp".into()), mem_x.clone()];
            prop_assert!(
                encode_ldrs(&sp_dest, size).is_err(),
                "SP dest must Err (llvm-mc: invalid operand); got {:?}",
                encode_ldrs(&sp_dest, size)
            );
            let wsp_dest = [Operand::Reg("wsp".into()), mem_x.clone()];
            prop_assert!(
                encode_ldrs(&wsp_dest, size).is_err(),
                "WSP dest must Err (llvm-mc: invalid operand); got {:?}",
                encode_ldrs(&wsp_dest, size)
            );
            let fp_dest = [Operand::Reg(format!("{}{}", simd, rt.min(31))), mem_x.clone()];
            prop_assert!(
                encode_ldrs(&fp_dest, size).is_err(),
                "SIMD dest must Err (llvm-mc: invalid operand); got {:?}",
                encode_ldrs(&fp_dest, size)
            );
            let w_base = [
                Operand::Reg(rt_name(rt, is_64)),
                Operand::Mem {
            ...
            prop_assert!(
                encode_ldrs(&w_base, size).is_err(),
                "W base must Err (llvm-mc: invalid operand); got {:?}",
                encode_ldrs(&w_base, size)
            );
            let xzr_base = [
                Operand::Reg(rt_name(rt, is_64)),
                Operand::Mem {
            ...
            prop_assert!(
                encode_ldrs(&xzr_base, size).is_err(),
                "XZR base must Err (llvm-mc: invalid operand); got {:?}",
                encode_ldrs(&xzr_base, size)
            );
            let w_index = [
                Operand::Reg(rt_name(rt, is_64)),
                Operand::MemRegOffset {
            ...
            prop_assert!(
                encode_ldrs(&w_index, size).is_err(),
                "W index without uxtw/sxtw must Err; got {:?}",
                encode_ldrs(&w_index, size)
            );
            if rt != 31 {
                let overlap_pre = [
                    Operand::Reg(rt_name(rt, is_64)),
                ...
                prop_assert!(
                    encode_ldrs(&overlap_pre, size).is_err(),
                    "pre-index Rt==Rn must Err (llvm-mc: unpredictable); got {:?}",
                    encode_ldrs(&overlap_pre, size)
                );
            }
        }
        ...
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_neg_offset_range_extra(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            rn in reg_edge(),
            off in offset_out_of_range(0),
            extra in extra_operand(),
            prepost in 0u32..=1,
        ) {
            // Re-draw a size-appropriate out-of-range offset: the strategy above is for byte;
            // for halfword, 4096 is a valid unsigned pimm, so coerce those cases.
            let off = if is_unsigned_pimm(off, size) || (off >= SIMM_MIN && off <= SIMM_MAX) {
                if size == 0 { 4096 } else { 8192 }
            } else {
                off
            };
            prop_assert!(
                !is_unsigned_pimm(off, size),
                "generator produced a valid unsigned pimm {}",
                off
            );
            prop_assert!(
            ...
            prop_assert!(
                encode_ldrs(&mem, size).is_err(),
                "out-of-range Mem offset {} must Err (llvm-mc range); got {:?}",
                off,
                encode_ldrs(&mem, size)
            );
            let three = vec![
                Operand::Reg(rt_name(rt, is_64)),
                Operand::Mem {
            ...
            prop_assert!(
                encode_ldrs(&three, size).is_err(),
                "extra operand must Err; got {:?}",
                encode_ldrs(&three, size)
            );
            let wb = if prepost == 0 {
                vec![
                    Operand::Reg(rt_name(rt, is_64)),
        ...
        // Oracle: differential (alt spellings x31 / uppercase / lr)
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_diff_alt_spellings(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            rn in reg_edge(),
            ...
            let (rt_s, rn_s, asm_rt, asm_rn) = match spelling {
                0 => {
                    let rt_s = if is_64 {
                        if rt == 31 { "x31".to_string() } else { format!("x{}", rt) }
                    } else if rt == 31 {
                        "w31".to_string()
                    } else {
                        format!("w{}", rt)
                ...
                1 => {
                    let rt_s = if is_64 {
                        if rt == 31 { "XZR".to_string() } else { format!("X{}", rt) }
                    } else if rt == 31 {
                        "WZR".to_string()
                    } else {
                        format!("W{}", rt)
            ...
            let asm = format!("{} {}, [{}]", mnemonic(size), asm_rt, asm_rn);
            let sut = sut_word(&ops, size)
                .unwrap_or_else(|e| panic!("SUT rejected valid {}: {}", asm, e));
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid {}: {}", asm, e));
        ...
        // Oracle: negative_error (unsupported extend/shift and bad base)
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_neg_bad_extend_base(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            rn in 0u32..=30,
            ...
            let ops = [Operand::Reg(rt_name(rt, is_64)), mem];
            prop_assert!(
                encode_ldrs(&ops, size).is_err(),
                "unsupported extend/shift or invalid base must Err; got {:?}",
                encode_ldrs(&ops, size)
        ...
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_neg_fp_dest(
            size in size_bit(),
            rt in 0u32..=31,
            simd in prop_oneof![Just('d'), Just('s'), Just('q'), Just('h'), Just('b')],
        ) {
            let ops = [
                Operand::Reg(format!("{}{}", simd, rt)),
                Operand::Mem { base: "x1".into(), offset: 0 },
            ];
            prop_assert!(
                encode_ldrs(&ops, size).is_err(),
                "SIMD dest must Err; got {:?}",
                encode_ldrs(&ops, size)
            );
        }
        ...
        fn encode_ldrs_neg_w_base(
            ...
            let ops = [
                Operand::Reg(rt_name(rt, is_64)),
                Operand::Mem { base: wt_name(rn), offset: 0 },
            ];
            prop_assert!(
                encode_ldrs(&ops, size).is_err(),
                "W base must Err; got {:?}",
        ...
        fn encode_ldrs_neg_w_index(
            ...
            prop_assert!(
                encode_ldrs(&ops, size).is_err(),
                "W index without extend must Err; got {:?}",
                encode_ldrs(&ops, size)
            );
        }
        ...
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_neg_writeback_overlap(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in 0u32..=30,
        ) {
            ...
            prop_assert!(
                encode_ldrs(&ops, size).is_err(),
                "pre-index Rt==Rn must Err; got {:?}",
                encode_ldrs(&ops, size)
            );
        }

        // Oracle: negative_error (coverage-gaps sweep: invalid names / bad base)
        // Target: encoder.load_store.encode_ldrs
        #[test]
        fn encode_ldrs_neg_invalid_name(
            size in size_bit(),
            is_64 in any::<bool>(),
            rt in reg_edge(),
            bad in prop_oneof![
            ...
            let dest_bad = [Operand::Reg(bad.clone()), mem_ok];
            prop_assert!(
                encode_ldrs(&dest_bad, size).is_err(),
                "invalid dest name {:?} must Err; got {:?}",
                bad,
                encode_ldrs(&dest_bad, size)
            );
            let base_bad = [
                Operand::Reg(rt_name(rt, is_64)),
                Operand::Mem {
            ...
            prop_assert!(
                encode_ldrs(&base_bad, size).is_err(),
                "invalid base name {:?} must Err; got {:?}",
                bad,
                encode_ldrs(&base_bad, size)
            );
        }
    }

    #[test]
    fn test_encode_ldrs_regression_sp_dest() {
        let ops = [
            Operand::Reg("sp".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb sp, [x1] must Err; llvm-mc: invalid operand (dest is Wt|Xt, 31=ZR)"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_wsp_dest() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb wsp, [x1] must Err; llvm-mc: invalid operand"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_fp_dest() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb d0, [x1] must Err; llvm-mc: invalid operand (dest is Wt|Xt)"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_w_base() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "w1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [w1] must Err; llvm-mc: invalid operand (base is Xn|SP)"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_xzr_base() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "xzr".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [xzr] must Err; llvm-mc: invalid operand (base is Xn|SP)"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_w_index_no_extend() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::MemRegOffset {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [x1, w2] must Err; llvm-mc requires uxtw/sxtw on Wm"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_writeback_rt_eq_rn() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::MemPreIndex {
                base: "x0".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [x0, #4]! must Err; llvm-mc: unpredictable writeback base is also a source"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_imm9_range() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [x1, #-257] must Err; llvm-mc requires simm9 in [-256, 255] when not a valid pimm"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_pimm_overflow() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [x1, #4096] must Err; unsigned imm12 max is 4095 (byte)"
        );
        let ops_h = [
            Operand::Reg("x0".into()),
            Operand::Mem {
        ...
        assert!(
            encode_ldrs(&ops_h, 1).is_err(),
            "ldrsh x0, [x1, #8192] must Err; unsigned imm12 max is 4095 (offset 8190)"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_extra_operand() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Mem {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [x1], x2 as 3 operands must Err; llvm-mc rejects a 3rd operand"
        );
    }

    #[test]
    fn test_encode_ldrs_regression_bad_shift() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::MemRegOffset {
                base: "x1".into(),
        ...
        assert!(
            encode_ldrs(&ops, 0).is_err(),
            "ldrsb x0, [x1, x2, lsl #1] must Err; llvm-mc: shift of #0 only for byte"
        );
    }
}
