...
mod encode_fp_arith_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:377-404 fadd/fsub/fmul/fdiv/fmax/fmin/fmaxnm/fminnm scalar dispatch to encode_fp_arith;
    //   ARM ARM Floating-point data-processing (2 source):
    //   M=0 S=0 11110 ftype 1 Rm opcode 10 Rn Rd;
    //   fp_scalar.rs:76 purpose comment (0 00 11110 ftype 1 Rm opcode 10 Rn Rd);
    //   README.md:223 lists the 8 scalar mnemonics.
    // Stronger considered:
    //   - State machine: rejected — encode_fp_arith is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree FP 2-source decoder
    //   - encode_neon_float_three_same as differential sibling:
    //     rejected — same-job gate fails (vector RegArrangement form)
    //   - encode_fmadd_fmsub / encode_fnmadd_fnmsub: rejected — 4-operand fused class
    //   - encode_fp_1src / encode_fneg / encode_fabs / encode_fsqrt: rejected — 1-source
    // Weaker available: algebraic.metamorphic (ftype/opcode/Rd/Rn/Rm),
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / wrong type)
    // Differential: candidate=encode_fp_arith, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler (caller-reachable from encode_instruction),
    //   mapping=[Reg(Sd|Dd), Reg(Sn|Dn), Reg(Sm|Dm)]+opcode <-> `fadd|fsub|fmul|fdiv|fmax|fmin|fmaxnm|fminnm Sd|Dd, Sn|Dn, Sm|Dm`

    use super::encode_fp_arith;
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    /// Documented scalar FP 2-source (opcode, mnemonic) pairs from encoder/mod.rs:377-404.
    const ARITH: &[(u32, &str)] = &[
        (0b0000, "fmul"),
        (0b0001, "fdiv"),
        (0b0010, "fadd"),
    ...
    fn sut_word(ops: &[Operand], opcode: u32) -> Result<u32, String> {
        match encode_fp_arith(ops, opcode)? {
            EncodeResult::Word(w) => Ok(w),
            other => Err(format!("expected Word, got {:?}", other)),
        }
    }
    ...
    fn wrong_type_triple() -> impl Strategy<Value = (String, String, String)> {
        let n = 0u32..=31;
        prop_oneof![
            // mixed S/D on one slot
            (n.clone(), n.clone(), n.clone(), 0u32..=2).prop_map(|(d, s, m, slot)| {
                let mut t = [fp(false, d), fp(false, s), fp(false, m)];
                let nums = [d, s, m];
                t[slot as usize] = fp(true, nums[slot as usize]);
                (t[0].clone(), t[1].clone(), t[2].clone())
            }),
            // GPR in one slot, matching S elsewhere
            (n.clone(), n.clone(), n.clone(), 0u32..=2, any::<bool>()).prop_map(|(d, s, m, slot, is64)| {
                let mut t = [fp(false, d), fp(false, s), fp(false, m)];
                let nums = [d, s, m];
            ...
            // Q/V/B in one slot
            (n.clone(), n.clone(), n.clone(), 0u32..=2, 0u32..=2).prop_map(|(d, s, m, slot, p)| {
                let pref = ["q", "v", "b"][p as usize];
                let mut t = [fp(false, d), fp(false, s), fp(false, m)];
                let nums = [d, s, m];
            ...
            // all-GPR
            (n.clone(), n.clone(), n.clone(), any::<bool>()).prop_map(|(d, s, m, is64)| {
                (gpr(is64, d), gpr(is64, s), gpr(is64, m))
            }),
        ]
    }
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_fp_arith_kat_llvm_mc_fadd_s0_s1_s2() {
        let want = 0x1e222820u32;
        let mc = llvm_mc_word("fadd s0, s1, s2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
        ...
        let sut = sut_word(&ops, 0b0010).expect("SUT KAT");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_fp_arith_kat_llvm_mc_fadd_d0_d1_d2() {
        let want = 0x1e622820u32;
        let mc = llvm_mc_word("fadd d0, d1, d2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("d1".into()),
    ...
    #[test]
    fn encode_fp_arith_kat_llvm_mc_fsub_s0_s1_s2() {
        let want = 0x1e223820u32;
        let mc = llvm_mc_word("fsub s0, s1, s2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
        ...
        let sut = sut_word(&ops, 0b0011).expect("SUT KAT");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_fp_arith_kat_llvm_mc_fmul_s0_s1_s2() {
        let want = 0x1e220820u32;
        let mc = llvm_mc_word("fmul s0, s1, s2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
        ...
        let sut = sut_word(&ops, 0b0000).expect("SUT KAT");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_fp_arith_kat_llvm_mc_fdiv_s0_s1_s2() {
        let want = 0x1e221820u32;
        let mc = llvm_mc_word("fdiv s0, s1, s2").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
        ...
        let sut = sut_word(&ops, 0b0001).expect("SUT KAT");
        assert_eq!(sut, want);
    }

    #[test]
    fn encode_fp_arith_kat_llvm_mc_fmax_fmin_nm_s0_s1_s2() {
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
            Operand::Reg("s2".into()),
        ];
        for (opcode, mnem, want) in [
            (0b0100u32, "fmax", 0x1e224820u32),
            (0b0101, "fmin", 0x1e225820),
            (0b0110, "fmaxnm", 0x1e226820),
            (0b0111, "fminnm", 0x1e227820),
        ] {
            let mc = llvm_mc_word(&format!("{mnem} s0, s1, s2")).expect("llvm-mc KAT");
            assert_eq!(mc, want, "llvm-mc KAT mapping broken for {mnem}");
            let sut = sut_word(&ops, opcode).expect("SUT KAT");
            assert_eq!(sut, want, "SUT KAT for {mnem}");
        }
    }

    #[test]
    fn encode_fp_arith_kat_llvm_mc_fadd_s31_s31_s31() {
        let want = 0x1e3f2bffu32;
        let mc = llvm_mc_word("fadd s31, s31, s31").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
    ...
    #[test]
    fn encode_fp_arith_kat_llvm_mc_half_fadd_h0_h1_h2() {
        let want = 0x1ee22820u32;
        let mc = llvm_mc_fp16_word("fadd h0, h1, h2").expect("llvm-mc fp16 KAT");
        assert_eq!(mc, want, "llvm-mc fp16 KAT mapping broken");
    }
    ...
    fn test_encode_fp_arith_regression_extra_operand() {
        ...
        assert!(
            ...
            "scalar FP arith must reject a 4th operand"
        );
    }

    #[test]
    fn test_encode_fp_arith_regression_mixed_sd() {
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("d1".into()),
            Operand::Reg("s2".into()),
        ...
        assert!(
            encode_fp_arith(&ops, 0b0010).is_err(),
            "scalar FP arith must reject mixed S/D operands"
        );
    }

    #[test]
    fn test_encode_fp_arith_regression_half_ftype() {
        let ops = [
            Operand::Reg("h0".into()),
            Operand::Reg("h1".into()),
            Operand::Reg("h2".into()),
        ];
        let sut = sut_word(&ops, 0b0010).expect("H,H,H is a valid fp16 FADD");
        assert_eq!(
            sut, 0x1ee22820u32,
            "H registers must use ftype=11 (0x1ee22820), not ftype=00 S"
        );
    }

    #[test]
    fn test_encode_fp_arith_regression_gpr() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("s1".into()),
            Operand::Reg("s2".into()),
        ...
        assert!(
            encode_fp_arith(&ops, 0b0010).is_err(),
            "scalar FP arith must reject GPR operands"
        );
    }

    #[test]
    fn test_encode_fp_arith_regression_sp() {
        let ops = [
            Operand::Reg("sp".into()),
            Operand::Reg("s1".into()),
            Operand::Reg("s2".into()),
        ...
        assert!(
            encode_fp_arith(&ops, 0b0010).is_err(),
            "SP is not a valid scalar FP arith register"
        );
    }
    ...
    proptest! {
        ...
        #[test]
        fn encode_fp_arith_diff_valid(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            is_d in any::<bool>(),
            idx in 0usize..ARITH.len(),
            dest_kind in 0u32..=1,
            src_kind in 0u32..=1,
            rm_kind in 0u32..=1,
        ) {
            let (opcode, mnem) = ARITH[idx];
            let dest = fp_spelling(is_d, rd, dest_kind);
            let src = fp_spelling(is_d, rn, src_kind);
            let src2 = fp_spelling(is_d, rm, rm_kind);
            let asm = format!("{mnem} {dest}, {src}, {src2}");
            let ops = [
                Operand::Reg(dest.clone()),
                Operand::Reg(src.clone()),
                Operand::Reg(src2.clone()),
            ];
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid {asm}: {e}"));
            let sut = sut_word(&ops, opcode)
                .unwrap_or_else(|e| panic!("SUT rejected valid {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FP arith mismatch for {}", asm);
        }

        #[test]
        fn encode_fp_arith_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            is_d in any::<bool>(),
            ...
            let want = (0b00011110u32 << 24)
                ...
                | (rn << 5)
                | rd;
            prop_assert_eq!(w, want, "ARM ARM FP 2-source field layout");
            prop_assert_eq!((w >> 24) & 0xff, 0b00011110, "bits[31:24] M=0 S=0 11110");
            prop_assert_eq!((w >> 22) & 0b11, ftype, "ftype");
            prop_assert_eq!((w >> 21) & 1, 1, "bit21 must be 1");
            prop_assert_eq!((w >> 16) & 0x1f, rm, "Rm");
            prop_assert_eq!((w >> 12) & 0xf, opcode, "opcode bits[15:12]");
            prop_assert_eq!((w >> 10) & 0b11, 0b10, "bits[11:10] must be 10");
            prop_assert_eq!((w >> 5) & 0x1f, rn, "Rn");
            prop_assert_eq!(w & 0x1f, rd, "Rd");
        }

        #[test]
        fn encode_fp_arith_metamorphic_fields(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            is_d in any::<bool>(),
        ) {
            let ops = |d: u32, n: u32, m: u32, sd: bool| {
                [
                    Operand::Reg(fp(sd, d)),
                    Operand::Reg(fp(sd, n)),
                    Operand::Reg(fp(sd, m)),
            ...
            let w = sut_word(&ops(rd, rn, rm, is_d), 0b0010).expect("base FADD");
            let w_rd = sut_word(&ops(rd + 1, rn, rm, is_d), 0b0010).expect("Rd+1");
            let w_rn = sut_word(&ops(rd, rn + 1, rm, is_d), 0b0010).expect("Rn+1");
            let w_rm = sut_word(&ops(rd, rn, rm + 1, is_d), 0b0010).expect("Rm+1");
            let w_ft = sut_word(&ops(rd, rn, rm, !is_d), 0b0010).expect("ftype flip");
            let w_sub = sut_word(&ops(rd, rn, rm, is_d), 0b0011).expect("FSUB");
            prop_assert_eq!(w_rd, w + 1, "Rd+1 must increment bits[4:0] only");
            prop_assert_eq!(w_rn, w + (1 << 5), "Rn+1 must increment bits[9:5] only");
            prop_assert_eq!(w_rm, w + (1 << 16), "Rm+1 must increment bits[20:16] only");
            prop_assert_eq!(w_ft ^ w, 1u32 << 22, "S vs D must flip only ftype bit 22");
            prop_assert_eq!(w_sub ^ w, 1u32 << 12, "FADD XOR FSUB must be opcode LSB bit 12");
        }

        #[test]
        fn encode_fp_arith_neg_arity(
            len in 0usize..=2,
            n in 0u32..=31,
        ) {
            let mut ops = Vec::new();
            ...
            prop_assert!(
                encode_fp_arith(&ops, 0b0010).is_err(),
                "FP arith with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_fp_arith_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            is_d in any::<bool>(),
            ...
            prop_assert!(
                encode_fp_arith(&ops, 0b0010).is_err(),
                "scalar FP arith has no 4th operand; extra must Err"
            );
        }

        #[test]
        fn encode_fp_arith_neg_wrong_types(
            (dest, src, src2) in wrong_type_triple(),
        ) {
            let ops = [
                Operand::Reg(dest.clone()),
                Operand::Reg(src.clone()),
            ...
            prop_assert!(
                encode_fp_arith(&ops, 0b0010).is_err(),
                "FP arith requires matching Sd,Sn,Sm or Dd,Dn,Dm; dest={} src={} src2={} must Err",
                dest,
                src,
                src2
            );
        ...
        fn encode_fp_arith_diff_half(
            ...
            let sut = sut_word(&ops, opcode)
                .unwrap_or_else(|e| panic!("SUT rejected valid fp16 {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FP arith half-precision mismatch for {}", asm);
        }

        #[test]
        fn encode_fp_arith_neg_nonreg(
            which in 0u32..=2,
            kind in 0u32..=5,
        ) {
            let bad = match kind {
        ...
        #[test]
        fn encode_fp_arith_neg_invalid_name(
            which in 0u32..=2,
            name in prop::sample::select(vec![
                "foo", "s32", "d32", "h32", "x32", "r0", "s", "d", "",
            ]),
...
mod encode_fabs_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:408-411 scalar fabs (non-RegArrangement) dispatch to encode_fabs;
    //   ARM ARM Floating-point data-processing (1 source) FABS:
    //   M=0 S=0 11110 ftype 1 opcode=000001 10000 Rn Rd;
    //   fp_scalar.rs:98 purpose comment (FABS: 0 00 11110 ftype 1 0000 01 10000 Rn Rd);
    //   README.md:223 lists scalar fabs.
    // Stronger considered:
    //   - State machine: rejected — encode_fabs is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree FABS decoder
    //   - encode_fneg / encode_fsqrt as differential sibling:
    //     rejected — same-job gate fails (different ARM opcodes 000010 / 000011)
    ...
    // Differential: candidate=encode_fabs, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler (caller-reachable from encode_instruction),
    //   mapping=[Reg(Sd|Dd|Hd), Reg(Sn|Dn|Hn)] <-> `fabs Sd|Dd|Hd, Sn|Dn|Hn`

    use super::encode_fabs;
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    const LLVM_MC: &str = "/home/toan/tools/llvm15-official/bin/llvm-mc";
    const CASES: u32 = 1000;
    /// ARM ARM FABS 1-source opcode bits[20:15].
    const FABS_OPCODE: u32 = 0b000001;
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_fabs_kat_llvm_mc_s0_s1() {
        let want = 0x1e20c020u32;
        let mc = llvm_mc_word("fabs s0, s1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("s0".into()), Operand::Reg("s1".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fabs_kat_llvm_mc_d0_d1() {
        let want = 0x1e60c020u32;
        let mc = llvm_mc_word("fabs d0, d1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("d0".into()), Operand::Reg("d1".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fabs_kat_llvm_mc_s31_s31() {
        let want = 0x1e20c3ffu32;
        let mc = llvm_mc_word("fabs s31, s31").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("s31".into()), Operand::Reg("s31".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fabs_kat_llvm_mc_d31_d0() {
        let want = 0x1e60c01fu32;
        let mc = llvm_mc_word("fabs d31, d0").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("d31".into()), Operand::Reg("d0".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fabs_kat_llvm_mc_uppercase() {
        let want = 0x1e20c020u32;
        let mc = llvm_mc_word("fabs S0, S1").expect("llvm-mc uppercase KAT");
    ...
    #[test]
    fn encode_fabs_kat_llvm_mc_half_h0_h1() {
        let want = 0x1ee0c020u32;
        let mc = llvm_mc_fp16_word("fabs h0, h1").expect("llvm-mc fp16 KAT");
        assert_eq!(mc, want, "llvm-mc fp16 KAT mapping broken");
    }

    #[test]
    fn test_encode_fabs_regression_extra_operand() {
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s0".into()),
            Operand::Reg("s0".into()),
        ...
        assert!(
            ...
            "FABS must reject a 3rd operand"
        );
    }

    #[test]
    fn test_encode_fabs_regression_mixed_sd() {
        let ops = [Operand::Reg("s0".into()), Operand::Reg("d0".into())];
        assert!(
            encode_fabs(&ops).is_err(),
            "FABS must reject mixed S/D operands"
        );
    }

    #[test]
    fn test_encode_fabs_regression_half_ftype() {
        let ops = [Operand::Reg("h0".into()), Operand::Reg("h0".into())];
        let sut = sut_word(&ops).expect("H,H is a valid fp16 FABS");
        assert_eq!(
            sut, 0x1ee0c000u32,
            "H registers must use ftype=11 (0x1ee0c000), not ftype=00 S"
        );
    }
    ...
    proptest! {
        ...
        fn encode_fabs_diff_valid(
            ...
            let dest = fp_spelling(is_d, rd, dest_kind);
            let src = fp_spelling(is_d, rn, src_kind);
            let asm = format!("fabs {dest}, {src}");
            let ops = [Operand::Reg(dest.clone()), Operand::Reg(src.clone())];
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid {asm}: {e}"));
            let sut = sut_word(&ops)
                .unwrap_or_else(|e| panic!("SUT rejected valid {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FABS mismatch for {}", asm);
        }

        #[test]
        fn encode_fabs_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_d in any::<bool>(),
        ) {
            ...
            let want = (0b00011110u32 << 24)
                ...
                | (rn << 5)
                | rd;
            prop_assert_eq!(w, want, "ARM ARM FP 1-source FABS field layout");
            prop_assert_eq!((w >> 24) & 0xff, 0b00011110, "bits[31:24] M=0 S=0 11110");
            prop_assert_eq!((w >> 22) & 0b11, ftype, "ftype");
            prop_assert_eq!((w >> 21) & 1, 1, "bit21 must be 1");
            prop_assert_eq!((w >> 15) & 0x3f, FABS_OPCODE, "opcode bits[20:15] must be 000001");
            prop_assert_eq!((w >> 10) & 0x1f, 0b10000, "bits[14:10] must be 10000");
            prop_assert_eq!((w >> 5) & 0x1f, rn, "Rn");
            prop_assert_eq!(w & 0x1f, rd, "Rd");
        }
        ...
        #[test]
        fn encode_fabs_metamorphic_fields(
            rd in 0u32..=30,
            rn in 0u32..=30,
            is_d in any::<bool>(),
        ) {
            ...
            let w = sut_word(&ops(rd, rn, is_d)).expect("base FABS");
        ...
        fn encode_fabs_neg_arity(
            ...
            prop_assert!(
                encode_fabs(&ops).is_err(),
                "FABS with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_fabs_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_d in any::<bool>(),
            extra in extra_operand(),
            ...
            prop_assert!(
                encode_fabs(&ops).is_err(),
                "FABS has no 3rd operand; extra must Err"
            );
        }

        #[test]
        fn encode_fabs_neg_wrong_types(
            (dest, src) in wrong_type_pair(),
        ) {
            let ops = [Operand::Reg(dest.clone()), Operand::Reg(src.clone())];
            prop_assert!(
                encode_fabs(&ops).is_err(),
                "FABS requires matching Sd,Sn or Dd,Dn; dest={} src={} must Err",
                dest,
                src
            );
        }
        ...
        fn encode_fabs_diff_half(
            ...
            let sut = sut_word(&ops)
                .unwrap_or_else(|e| panic!("SUT rejected valid fp16 {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FABS half-precision mismatch for {}", asm);
        }

        #[test]
        fn encode_fabs_neg_nonreg(
            which in 0u32..=1,
            kind in 0u32..=5,
        ) {
            let bad = match kind {
        ...
        #[test]
        fn encode_fabs_neg_invalid_name(
            which in 0u32..=1,
            name in prop::sample::select(vec![
                "foo", "s32", "d32", "h32", "x32", "r0", "s", "d", "",
            ]),
...
#[cfg(test)]
mod encode_fmadd_fmsub_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:435-436 fmadd/fmsub dispatch to encode_fmadd_fmsub;
    //   ARM ARM Floating-point data-processing (3 source):
    //   M=0 S=0 11111 ftype o1 Rm o0 Ra Rn Rd;
    //   FMADD o1=0 o0=0, FMSUB o1=0 o0=1; ftype 00=S, 01=D, 11=H;
    //   fp_scalar.rs:127-128 purpose comment (Rd = Ra +/- (Rn * Rm);
    //   Format: 0 00 11111 ftype 0 Rm o1 Ra Rn Rd);
    //   README.md:223 lists scalar fmadd/fmsub.
    // Stronger considered:
    //   - State machine: rejected — encode_fmadd_fmsub is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree FMADD/FMSUB decoder
    //   - encode_fnmadd_fnmsub as differential sibling:
    //     rejected — same-job gate fails (o1=1 negated fused class)
    ...
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / wrong type / nonreg)
    // Differential: candidate=encode_fmadd_fmsub, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler (caller-reachable from encode_instruction),
    //   mapping=[Reg(Sd|Dd|Hd), Reg(Sn|Dn|Hn), Reg(Sm|Dm|Hm), Reg(Sa|Da|Ha)]+is_sub
    //   <-> `fmadd|fmsub Sd|Dd|Hd, Sn|Dn|Hn, Sm|Dm|Hm, Sa|Da|Ha`

    use super::encode_fmadd_fmsub;
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    fn sut_word(ops: &[Operand], is_sub: bool) -> Result<u32, String> {
        match encode_fmadd_fmsub(ops, is_sub)? {
            EncodeResult::Word(w) => Ok(w),
            other => Err(format!("expected Word, got {:?}", other)),
        }
    }
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_fmadd_fmsub_kat_llvm_mc_fmadd_s0_s1_s2_s3() {
        let want = 0x1f020c20u32;
        let mc = llvm_mc_word("fmadd s0, s1, s2, s3").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
    ...
    #[test]
    fn encode_fmadd_fmsub_kat_llvm_mc_fmadd_d0_d1_d2_d3() {
        let want = 0x1f420c20u32;
        let mc = llvm_mc_word("fmadd d0, d1, d2, d3").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("d1".into()),
    ...
    #[test]
    fn encode_fmadd_fmsub_kat_llvm_mc_fmsub_s0_s1_s2_s3() {
        let want = 0x1f028c20u32;
        let mc = llvm_mc_word("fmsub s0, s1, s2, s3").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
    ...
    #[test]
    fn encode_fmadd_fmsub_kat_llvm_mc_fmsub_d0_d1_d2_d3() {
        let want = 0x1f428c20u32;
        let mc = llvm_mc_word("fmsub d0, d1, d2, d3").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("d1".into()),
    ...
    #[test]
    fn encode_fmadd_fmsub_kat_llvm_mc_fmadd_s31_s31_s31_s31() {
        let want = 0x1f1f7fffu32;
        let mc = llvm_mc_word("fmadd s31, s31, s31, s31").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
            Operand::Reg("s31".into()),
            Operand::Reg("s31".into()),
    ...
    #[test]
    fn encode_fmadd_fmsub_kat_llvm_mc_uppercase() {
        let want = 0x1f020c20u32;
        let mc = llvm_mc_word("fmadd S0, S1, S2, S3").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [
    ...
    #[test]
    fn encode_fmadd_fmsub_kat_llvm_mc_half_fmadd_h0_h1_h2_h3() {
        let want = 0x1fc20c20u32;
        let mc = llvm_mc_fp16_word("fmadd h0, h1, h2, h3").expect("llvm-mc fp16 KAT");
        assert_eq!(mc, want, "llvm-mc fp16 KAT mapping broken");
    }

    #[test]
    fn test_encode_fmadd_fmsub_regression_extra_operand() {
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
            Operand::Reg("s2".into()),
        ...
        assert!(
            ...
            "FMADD must reject a 5th operand"
        );
    }

    #[test]
    fn test_encode_fmadd_fmsub_regression_mixed_sd() {
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("d1".into()),
            Operand::Reg("s2".into()),
        ...
        assert!(
            encode_fmadd_fmsub(&ops, false).is_err(),
            "FMADD must reject mixed S/D operands"
        );
    }

    #[test]
    fn test_encode_fmadd_fmsub_regression_half_ftype() {
        let ops = [
            Operand::Reg("h0".into()),
            Operand::Reg("h1".into()),
            Operand::Reg("h2".into()),
        ...
        let sut = sut_word(&ops, false).expect("H,H,H,H is a valid fp16 FMADD");
        assert_eq!(
            sut, 0x1fc20c20u32,
            "H registers must use ftype=11 (0x1fc20c20), not ftype=00 S"
        );
    }

    #[test]
    fn test_encode_fmadd_fmsub_regression_gpr() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("s1".into()),
            Operand::Reg("s2".into()),
        ...
        assert!(
            encode_fmadd_fmsub(&ops, false).is_err(),
            "FMADD must reject GPR dest"
        );
    }

    #[test]
    fn test_encode_fmadd_fmsub_regression_sp() {
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s1".into()),
            Operand::Reg("s2".into()),
    ...
    proptest! {
        ...
        #[test]
        fn encode_fmadd_fmsub_diff_valid(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            let sut = sut_word(&ops, is_sub)
                .unwrap_or_else(|e| panic!("SUT rejected valid {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FMADD/FMSUB mismatch for {}", asm);
        }

        #[test]
        fn encode_fmadd_fmsub_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            let want = (0b00011111u32 << 24)
                ...
                | (rn << 5)
                | rd;
            prop_assert_eq!(w, want, "ARM ARM FP 3-source field layout");
            prop_assert_eq!((w >> 24) & 0xff, 0b00011111, "bits[31:24] M=0 S=0 11111");
            prop_assert_eq!((w >> 22) & 0b11, ftype, "ftype");
            prop_assert_eq!((w >> 21) & 1, 0, "o1/bit21 must be 0 for FMADD/FMSUB");
            prop_assert_eq!((w >> 16) & 0x1f, rm, "Rm");
        ...
        #[test]
        fn encode_fmadd_fmsub_metamorphic_fields(
            rd in 0u32..=30,
            rn in 0u32..=30,
            rm in 0u32..=30,
            ra in 0u32..=30,
            ...
            let w = sut_word(&ops(rd, rn, rm, ra, is_d), false).expect("base FMADD");
            ...
            let w_sub = sut_word(&ops(rd, rn, rm, ra, is_d), true).expect("FMSUB");
            prop_assert_eq!(w_rd, w + 1, "Rd+1 must increment bits[4:0] only");
            prop_assert_eq!(w_rn, w + (1 << 5), "Rn+1 must increment bits[9:5] only");
            prop_assert_eq!(w_ra, w + (1 << 10), "Ra+1 must increment bits[14:10] only");
            prop_assert_eq!(w_rm, w + (1 << 16), "Rm+1 must increment bits[20:16] only");
            prop_assert_eq!(w_ft ^ w, 1u32 << 22, "S vs D must flip only ftype bit 22");
            prop_assert_eq!(w_sub ^ w, 1u32 << 15, "FMADD XOR FMSUB must be o0 bit 15");
        }

        #[test]
        fn encode_fmadd_fmsub_neg_arity(
            len in 0usize..=3,
            n in 0u32..=31,
            is_sub in any::<bool>(),
        ) {
            ...
            prop_assert!(
                encode_fmadd_fmsub(&ops, is_sub).is_err(),
                "FMADD/FMSUB with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_fmadd_fmsub_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            prop_assert!(
                encode_fmadd_fmsub(&ops, is_sub).is_err(),
                "FMADD/FMSUB has no 5th operand; extra must Err"
            );
        }

        #[test]
        fn encode_fmadd_fmsub_neg_wrong_types(
            ...
            prop_assert!(
                encode_fmadd_fmsub(&ops, is_sub).is_err(),
                "FMADD/FMSUB requires matching Sd/Dd/Hd quadruples; dest={} n={} m={} a={} must Err",
                dest,
                src_n,
                src_m,
                src_a
        ...
        #[test]
        fn encode_fmadd_fmsub_diff_half(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rm in 0u32..=31,
            ra in 0u32..=31,
            ...
            let sut = sut_word(&ops, is_sub)
                .unwrap_or_else(|e| panic!("SUT rejected valid fp16 {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FMADD/FMSUB half-precision mismatch for {}", asm);
        }

        #[test]
        fn encode_fmadd_fmsub_neg_nonreg(
            which in 0u32..=3,
            kind in 0u32..=5,
            is_sub in any::<bool>(),
        ) {
        ...
        #[test]
        fn encode_fmadd_fmsub_neg_invalid_name(
            which in 0u32..=3,
            name in prop::sample::select(vec![
                "foo", "s32", "d32", "h32", "x32", "r0", "s", "d", "",
            ]),
...
#[cfg(test)]
mod encode_fsqrt_pbt {
    // Oracle: differential — llvm-mc AArch64 assembler
    // Evidence: src/backend/arm/assembler/README.md "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:411-413 scalar fsqrt (non-RegArrangement) dispatch to encode_fsqrt;
    //   ARM ARM Floating-point data-processing (1 source) FSQRT:
    //   M=0 S=0 11110 ftype 1 opcode=000011 10000 Rn Rd;
    //   fp_scalar.rs:111 purpose comment (FSQRT: 0 00 11110 ftype 1 0000 11 10000 Rn Rd);
    //   README.md:223 lists scalar fsqrt.
    // Stronger considered:
    //   - State machine: rejected — encode_fsqrt is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree FSQRT decoder
    //   - encode_fabs / encode_fneg as differential sibling:
    //     rejected — same-job gate fails (different ARM opcodes 000001 / 000010)
    //   - encode_fp_1src: rejected — FRINT* opcodes
    //   - encode_neon_float_two_misc: rejected — vector/SIMD-scalar form
    // Weaker available: algebraic.metamorphic (ftype/Rd/Rn),
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / wrong type / nonreg)
    // Differential: candidate=encode_fsqrt, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler (caller-reachable from encode_instruction),
    //   mapping=[Reg(Sd|Dd|Hd), Reg(Sn|Dn|Hn)] <-> `fsqrt Sd|Dd|Hd, Sn|Dn|Hn`

    use super::encode_fsqrt;
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    const LLVM_MC: &str = "/home/toan/tools/llvm15-official/bin/llvm-mc";
    const CASES: u32 = 1000;
    /// ARM ARM FSQRT 1-source opcode bits[20:15].
    const FSQRT_OPCODE: u32 = 0b000011;
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_fsqrt_kat_llvm_mc_s0_s1() {
        let want = 0x1e21c020u32;
        let mc = llvm_mc_word("fsqrt s0, s1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("s0".into()), Operand::Reg("s1".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fsqrt_kat_llvm_mc_d0_d1() {
        let want = 0x1e61c020u32;
        let mc = llvm_mc_word("fsqrt d0, d1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("d0".into()), Operand::Reg("d1".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fsqrt_kat_llvm_mc_s31_s31() {
        let want = 0x1e21c3ffu32;
        let mc = llvm_mc_word("fsqrt s31, s31").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("s31".into()), Operand::Reg("s31".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fsqrt_kat_llvm_mc_d31_d0() {
        let want = 0x1e61c01fu32;
        let mc = llvm_mc_word("fsqrt d31, d0").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let ops = [Operand::Reg("d31".into()), Operand::Reg("d0".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fsqrt_kat_llvm_mc_uppercase() {
        let want = 0x1e21c020u32;
        let mc = llvm_mc_word("fsqrt S0, S1").expect("llvm-mc uppercase KAT");
        assert_eq!(mc, want, "llvm-mc uppercase KAT mapping broken");
        let ops = [Operand::Reg("S0".into()), Operand::Reg("S1".into())];
        let sut = sut_word(&ops).expect("SUT KAT");
        assert_eq!(sut, want);
    ...
    #[test]
    fn encode_fsqrt_kat_llvm_mc_half_h0_h1() {
        let want = 0x1ee1c020u32;
        let mc = llvm_mc_fp16_word("fsqrt h0, h1").expect("llvm-mc fp16 KAT");
        assert_eq!(mc, want, "llvm-mc fp16 KAT mapping broken");
    }

    #[test]
    fn test_encode_fsqrt_regression_extra_operand() {
        let ops = [
            Operand::Reg("s0".into()),
            Operand::Reg("s0".into()),
            Operand::Reg("s0".into()),
        ...
        assert!(
            ...
            "FSQRT must reject a 3rd operand"
        );
    }

    #[test]
    fn test_encode_fsqrt_regression_mixed_sd() {
        let ops = [Operand::Reg("s0".into()), Operand::Reg("d0".into())];
        assert!(
            encode_fsqrt(&ops).is_err(),
            "FSQRT must reject mixed S/D operands"
        );
    }

    #[test]
    fn test_encode_fsqrt_regression_half_ftype() {
        let ops = [Operand::Reg("h0".into()), Operand::Reg("h0".into())];
        let sut = sut_word(&ops).expect("H,H is a valid fp16 FSQRT");
        assert_eq!(
            sut, 0x1ee1c000u32,
            "H registers must use ftype=11 (0x1ee1c000), not ftype=00 S"
        );
    }
    ...
    proptest! {
        ...
        fn encode_fsqrt_diff_valid(
            ...
            let dest = fp_spelling(is_d, rd, dest_kind);
            let src = fp_spelling(is_d, rn, src_kind);
            let asm = format!("fsqrt {dest}, {src}");
            let ops = [Operand::Reg(dest.clone()), Operand::Reg(src.clone())];
            let mc = llvm_mc_word(&asm)
                .unwrap_or_else(|e| panic!("llvm-mc rejected valid {asm}: {e}"));
            let sut = sut_word(&ops)
                .unwrap_or_else(|e| panic!("SUT rejected valid {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FSQRT mismatch for {}", asm);
        }

        #[test]
        fn encode_fsqrt_arm_fields(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_d in any::<bool>(),
        ) {
            ...
            let want = (0b00011110u32 << 24)
                ...
                | (rn << 5)
                | rd;
            prop_assert_eq!(w, want, "ARM ARM FP 1-source FSQRT field layout");
            prop_assert_eq!((w >> 24) & 0xff, 0b00011110, "bits[31:24] M=0 S=0 11110");
            prop_assert_eq!((w >> 22) & 0b11, ftype, "ftype");
            prop_assert_eq!((w >> 21) & 1, 1, "bit21 must be 1");
            prop_assert_eq!((w >> 15) & 0x3f, FSQRT_OPCODE, "opcode bits[20:15] must be 000011");
            prop_assert_eq!((w >> 10) & 0x1f, 0b10000, "bits[14:10] must be 10000");
            prop_assert_eq!((w >> 5) & 0x1f, rn, "Rn");
            prop_assert_eq!(w & 0x1f, rd, "Rd");
        }
        ...
        #[test]
        fn encode_fsqrt_metamorphic_fields(
            rd in 0u32..=30,
            rn in 0u32..=30,
            is_d in any::<bool>(),
        ) {
            ...
            let w = sut_word(&ops(rd, rn, is_d)).expect("base FSQRT");
            let w_rd = sut_word(&ops(rd + 1, rn, is_d)).expect("Rd+1");
            let w_rn = sut_word(&ops(rd, rn + 1, is_d)).expect("Rn+1");
            let w_ft = sut_word(&ops(rd, rn, !is_d)).expect("ftype flip");
            prop_assert_eq!(w_rd, w + 1, "Rd+1 must increment bits[4:0] only");
        ...
        fn encode_fsqrt_neg_arity(
            ...
            prop_assert!(
                encode_fsqrt(&ops).is_err(),
                "FSQRT with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_fsqrt_neg_extra_operand(
            rd in 0u32..=31,
            rn in 0u32..=31,
            is_d in any::<bool>(),
            extra in extra_operand(),
            ...
            prop_assert!(
                encode_fsqrt(&ops).is_err(),
                "FSQRT has no 3rd operand; extra must Err"
            );
        }

        #[test]
        fn encode_fsqrt_neg_wrong_types(
            (dest, src) in wrong_type_pair(),
        ) {
            let ops = [Operand::Reg(dest.clone()), Operand::Reg(src.clone())];
            prop_assert!(
                encode_fsqrt(&ops).is_err(),
                "FSQRT requires matching Sd,Sn or Dd,Dn; dest={} src={} must Err",
                dest,
                src
            );
        }
        ...
        fn encode_fsqrt_diff_half(
            ...
            let sut = sut_word(&ops)
                .unwrap_or_else(|e| panic!("SUT rejected valid fp16 {asm}: {e}"));
            prop_assert_eq!(sut, mc, "FSQRT half-precision mismatch for {}", asm);
        }

        #[test]
        fn encode_fsqrt_neg_nonreg(
            which in 0u32..=1,
            kind in 0u32..=5,
        ) {
            let bad = match kind {
        ...
        #[test]
        fn encode_fsqrt_neg_invalid_name(
            which in 0u32..=1,
            name in prop::sample::select(vec![
                "foo", "s32", "d32", "h32", "x32", "r0", "s", "d", "",
            ]),
