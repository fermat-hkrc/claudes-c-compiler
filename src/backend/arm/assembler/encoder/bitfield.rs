...
mod encode_ubfm_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md:11 "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:887 "ubfm" => encode_ubfm; README.md:216 lists ubfm;
    //   bitfield.rs:36 purpose comment Encode UBFM Rd, Rn, #immr, #imms (raw form);
    //   ARM ARM Bitfield Move UBFM: sf 10 100110 N immr imms Rn Rd,
    //   N=sf; 32-bit immr/imms in [0,31], 64-bit in [0,63]; register 31 is ZR not SP.
    // Stronger considered:
    //   - State machine: rejected — encode_ubfm is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree UBFM decoder
    //   - encode_ubfx / encode_ubfiz as differential sibling: rejected — same-job gate fails (alias lsb/width);
    //     encode_ubfx used only as algebraic alias after the ARM mapping
    //   - encode_sbfm / encode_bfm: rejected — SBFM/BFM opc, different instruction
    // Weaker available: algebraic.metamorphic (UBFX alias of UBFM; Rd/Rn field independence),
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / SP / immr-imms)
    // Differential: candidate=encode_ubfm, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=[Reg(Rd), Reg(Rn), Imm(immr), Imm(imms)] <-> `ubfm Rd, Rn, #immr, #imms`

    use super::{encode_ubfm, encode_ubfx};
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    fn ubfm_valid() -> impl Strategy<Value = (bool, u32, u32, u32, u32)> {
        any::<bool>().prop_flat_map(|is_64| {
            let r = if is_64 { 64u32 } else { 32 };
            (
                Just(is_64),
    ...
    fn ubfx_valid() -> impl Strategy<Value = (bool, u32, u32, u32, u32)> {
        any::<bool>().prop_flat_map(|is_64| {
            let r = if is_64 { 64u32 } else { 32 };
            (
                Just(is_64),
    ...
    fn ubfx_word(ops: &[Operand]) -> Result<u32, String> {
        match encode_ubfx(ops)? {
            EncodeResult::Word(w) => Ok(w),
            other => Err(format!("expected Word, got {:?}", other)),
        }
    }
    ...
    fn must_err(ops: &[Operand]) -> bool {
        match catch_unwind(AssertUnwindSafe(|| encode_ubfm(ops))) {
            Ok(Err(_)) => true,
            _ => false,
        }
    }
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_ubfm_kat_llvm_mc_w0_w1_immr0_imms0() {
        let want = 0x53000020u32;
        let mc = llvm_mc_word("ubfm w0, w1, #0, #0").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(false, 0, 1, 0, 0)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfm_kat_llvm_mc_w0_w1_immr1_imms0() {
        let want = 0x53010020u32;
        let mc = llvm_mc_word("ubfm w0, w1, #1, #0").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(false, 0, 1, 1, 0)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfm_kat_llvm_mc_x0_x1_immr1_imms8() {
        let want = 0xd3412020u32;
        let mc = llvm_mc_word("ubfm x0, x1, #1, #8").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(true, 0, 1, 1, 8)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfm_kat_llvm_mc_wzr_wzr_immr31_imms0() {
        let want = 0x531f03ffu32;
        let mc = llvm_mc_word("ubfm wzr, wzr, #31, #0").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(false, 31, 31, 31, 0)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfm_kat_llvm_mc_x0_xzr_immr63_imms63() {
        let want = 0xd37fffe0u32;
        let mc = llvm_mc_word("ubfm x0, xzr, #63, #63").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(true, 0, 31, 63, 63)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfm_kat_llvm_mc_lr_x1_immr8_imms16() {
        let want = 0xd348403eu32;
        let mc = llvm_mc_word("ubfm lr, x1, #8, #16").expect("llvm-mc LR KAT");
        assert_eq!(mc, want, "llvm-mc LR KAT mapping broken");
        let ops = [
            Operand::Reg("lr".into()),
            Operand::Reg("x1".into()),
    ...
    proptest! {
        ...
        #[test]
        fn encode_ubfm_diff_valid_gpr((is_64, rd, rn, immr, imms) in ubfm_valid()) {
            let dest = gpr(is_64, rd);
            let src = gpr(is_64, rn);
            let asm = format!("ubfm {}, {}, #{}, #{}", dest, src, immr, imms);
            let ops = ops4(is_64, rd, rn, immr as i64, imms as i64);
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UBFM mismatch for {}", asm);
        }

        #[test]
        fn encode_ubfm_alias_ubfx((is_64, rd, rn, lsb, width) in ubfx_valid()) {
            let imms = (lsb as i64) + (width as i64) - 1;
            let ubfm = sut_word(&ops4(is_64, rd, rn, lsb as i64, imms)).expect("UBFM");
            let ubfx = ubfx_word(&[
                Operand::Reg(gpr(is_64, rd)),
                Operand::Reg(gpr(is_64, rn)),
                Operand::Imm(lsb as i64),
            ...
            prop_assert_eq!(ubfm, ubfx, "UBFM #lsb,#(lsb+width-1) must alias UBFX #lsb,#width");
        }

        #[test]
        fn encode_ubfm_arm_fields((is_64, rd, rn, immr, imms) in ubfm_valid()) {
            let sf = if is_64 { 1u32 } else { 0 };
            let w = sut_word(&ops4(is_64, rd, rn, immr as i64, imms as i64)).expect("SUT");
            let want = (sf << 31)
                | (0b10 << 29)
                ...
                | (rn << 5)
                | rd;
            prop_assert_eq!(w, want, "ARM ARM UBFM field layout");
            prop_assert_eq!(w >> 31, sf, "sf");
            prop_assert_eq!((w >> 29) & 0b11, 0b10, "opc=10 UBFM");
            prop_assert_eq!((w >> 23) & 0x3f, 0b100110, "bits[28:23]");
            prop_assert_eq!((w >> 22) & 1, sf, "N=sf");
        ...
        #[test]
        fn encode_ubfm_metamorphic_rd_rn(
            is_64 in any::<bool>(),
            rd in 0u32..=30,
            rn in 0u32..=30,
            imm in 0u32..=1,
        ...
        fn encode_ubfm_neg_arity(
            ...
            ops.truncate(len);
            prop_assert!(
                encode_ubfm(&ops).is_err(),
                "UBFM with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_ubfm_neg_extra_operand(
            (is_64, rd, rn, immr, imms) in ubfm_valid(),
            extra in extra_operand(),
        ) {
            let mut ops = ops4(is_64, rd, rn, immr as i64, imms as i64).to_vec();
            ops.push(extra);
            prop_assert!(
                encode_ubfm(&ops).is_err(),
                "UBFM has no 5th operand; extra operand must Err (llvm-mc rejects it)"
            );
        }

        #[test]
        fn encode_ubfm_neg_sp(
            which in 0u32..=1,
            sp64 in any::<bool>(),
            is_64 in any::<bool>(),
            other in 0u32..=30,
            ...
            prop_assert!(
                encode_ubfm(&ops).is_err(),
                "SP/WSP is not a valid UBFM operand (which={} sp={})",
                which,
                sp
            );
        }
        ...
        fn encode_ubfm_neg_immr_imms(
            ...
            prop_assert!(
                must_err(&ops),
                "UBFM immr={} imms={} R={} must Err (ARM: 0<=immr,imms<R)",
                immr,
                imms,
                r
            );
        ...
        #[test]
        fn encode_ubfm_diff_alt_spellings(
            (is_64, rd, rn, immr, imms) in ubfm_valid(),
            dest_spell in 0u32..=4,
            src_spell in 0u32..=4,
        ) {
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UBFM alt-spelling mismatch for {}", asm);
        }

        #[test]
        fn encode_ubfm_neg_mixed_width(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rd64 in any::<bool>(),
            rn64 in any::<bool>(),
            ...
            prop_assert!(
                encode_ubfm(&ops).is_err(),
                "UBFM mixed W/X (rd64={} rn64={}) must Err (llvm-mc rejects it)",
                rd64,
                rn64
            );
        }
        ...
        fn encode_ubfm_neg_fp(
            ...
            prop_assert!(
                encode_ubfm(&ops).is_err(),
                "FP/SIMD register {} is not a valid UBFM operand (which={})",
                fp,
                which
            );
        }
        ...
        #[test]
        fn encode_ubfm_neg_invalid_name(
            which in 0u32..=1,
            name in invalid_name(),
        ) {
            let mut ops = ops4(false, 0, 1, 0, 0);
    ...
    #[test]
    fn test_encode_ubfm_regression_extra_operand() {
        let mut ops = ops4(false, 0, 0, 0, 0).to_vec();
        ops.push(Operand::Reg("x0".into()));
        assert!(
            encode_ubfm(&ops).is_err(),
            "UBFM w0, w0, #0, #0, x0 must Err; extra operand is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfm_regression_sp() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Reg("w0".into()),
            Operand::Imm(0),
        ...
        assert!(
            encode_ubfm(&ops).is_err(),
            "UBFM wsp, w0, #0, #0 must Err; register 31 is ZR not SP/WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfm_regression_immr_neg() {
        let ops = ops4(false, 0, 0, -1, 0);
        let result = catch_unwind(AssertUnwindSafe(|| encode_ubfm(&ops)));
        assert!(
            matches!(result, Ok(Err(_))),
            "UBFM w0, w0, #-1, #0 must Err; immr=-1 is outside [0,31] (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfm_regression_mixed_width() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("w0".into()),
            Operand::Imm(0),
        ...
        assert!(
            encode_ubfm(&ops).is_err(),
            "UBFM x0, w0, #0, #0 must Err; mixed W/X is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfm_regression_fp() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("x1".into()),
            Operand::Imm(0),
        ...
        assert!(
            encode_ubfm(&ops).is_err(),
            "UBFM d0, x1, #0, #0 must Err; FP/SIMD registers are not UBFM operands (llvm-mc rejects it)"
        );
    }
}
...
mod encode_ubfx_pbt {
    ...
    // Evidence: src/backend/arm/assembler/README.md:11 "accepts the same textual assembly that GCC's gas would consume";
    //   encoder/mod.rs "Encodes AArch64 instructions into 32-bit machine code words";
    //   encoder/mod.rs:885 "ubfx" => encode_ubfx; README.md:216 lists ubfx;
    //   bitfield.rs:6 purpose comment UBFX -> UBFM #lsb, #(lsb+width-1);
    //   ARM ARM Unsigned Bitfield Extract UBFX alias of UBFM: sf 10 100110 N immr imms Rn Rd,
    //   N=sf, immr=lsb, imms=lsb+width-1; register 31 is ZR not SP.
    // Stronger considered:
    //   - State machine: rejected — encode_ubfx is a pure function with no lifecycle
    //   - Algebraic round-trip: rejected — no in-tree UBFX decoder
    //   - encode_ubfm as differential sibling: rejected — same-job gate fails (raw immr/imms form);
    //     used only as algebraic alias after the ARM mapping
    //   - encode_sbfx / encode_bfxil: rejected — SBFM/BFM opc, different instruction
    //   - encode_ubfiz: rejected — different alias mapping (immr=(-lsb MOD R), imms=width-1)
    // Weaker available: algebraic.metamorphic (UBFX alias of UBFM; Rd/Rn field independence),
    //   algebraic.invariant (ARM fields), negative_error (arity / extra / SP / lsb-width)
    // Differential: candidate=encode_ubfx, reference=llvm-mc -triple=aarch64 -show-encoding,
    //   SUT-boundary=internal-helper of GNU-style assembler,
    //   mapping=[Reg(Rd), Reg(Rn), Imm(lsb), Imm(width)] <-> `ubfx Rd, Rn, #lsb, #width`

    use super::{encode_ubfx, encode_ubfm};
    use super::super::EncodeResult;
    use crate::backend::arm::assembler::parser::Operand;
    use proptest::prelude::*;
    use std::io::Write;
    ...
    fn must_err(ops: &[Operand]) -> bool {
        match catch_unwind(AssertUnwindSafe(|| encode_ubfx(ops))) {
            Ok(Err(_)) => true,
            _ => false,
        }
    }
    ...
    /// Known-answer gate for the llvm-mc differential connection.
    #[test]
    fn encode_ubfx_kat_llvm_mc_w0_w1_lsb0_width1() {
        let want = 0x53000020u32;
        let mc = llvm_mc_word("ubfx w0, w1, #0, #1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(false, 0, 1, 0, 1)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfx_kat_llvm_mc_w0_w1_lsb1_width1() {
        let want = 0x53010420u32;
        let mc = llvm_mc_word("ubfx w0, w1, #1, #1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(false, 0, 1, 1, 1)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfx_kat_llvm_mc_x0_x1_lsb1_width8() {
        let want = 0xd3412020u32;
        let mc = llvm_mc_word("ubfx x0, x1, #1, #8").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(true, 0, 1, 1, 8)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfx_kat_llvm_mc_wzr_wzr_lsb31_width1() {
        let want = 0x531f7fffu32;
        let mc = llvm_mc_word("ubfx wzr, wzr, #31, #1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(false, 31, 31, 31, 1)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfx_kat_llvm_mc_x0_xzr_lsb63_width1() {
        let want = 0xd37fffe0u32;
        let mc = llvm_mc_word("ubfx x0, xzr, #63, #1").expect("llvm-mc KAT");
        assert_eq!(mc, want, "llvm-mc KAT mapping broken");
        let sut = sut_word(&ops4(true, 0, 31, 63, 1)).expect("SUT KAT");
        assert_eq!(sut, want);
    }
    ...
    #[test]
    fn encode_ubfx_kat_llvm_mc_lr_x1_lsb8_width16() {
        let want = 0xd3485c3eu32;
        let mc = llvm_mc_word("ubfx lr, x1, #8, #16").expect("llvm-mc LR KAT");
        assert_eq!(mc, want, "llvm-mc LR KAT mapping broken");
        let ops = [
            Operand::Reg("lr".into()),
            Operand::Reg("x1".into()),
    ...
    proptest! {
        ...
        #[test]
        fn encode_ubfx_diff_valid_gpr((is_64, rd, rn, lsb, width) in ubfx_valid()) {
            let dest = gpr(is_64, rd);
            let src = gpr(is_64, rn);
            let asm = format!("ubfx {}, {}, #{}, #{}", dest, src, lsb, width);
            let ops = ops4(is_64, rd, rn, lsb as i64, width as i64);
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UBFX mismatch for {}", asm);
        }

        #[test]
        fn encode_ubfx_alias_ubfm((is_64, rd, rn, lsb, width) in ubfx_valid()) {
            let imms = (lsb as i64) + (width as i64) - 1;
            let ubfx = sut_word(&ops4(is_64, rd, rn, lsb as i64, width as i64)).expect("UBFX");
            let ubfm = ubfm_word(&[
                Operand::Reg(gpr(is_64, rd)),
                Operand::Reg(gpr(is_64, rn)),
                Operand::Imm(lsb as i64),
            ...
            prop_assert_eq!(ubfx, ubfm, "UBFX must alias UBFM #lsb, #(lsb+width-1)");
        }

        #[test]
        fn encode_ubfx_arm_fields((is_64, rd, rn, lsb, width) in ubfx_valid()) {
            let immr = lsb;
            let imms = lsb + width - 1;
            let sf = if is_64 { 1u32 } else { 0 };
            let w = sut_word(&ops4(is_64, rd, rn, lsb as i64, width as i64)).expect("SUT");
            let want = (sf << 31)
                ...
                | (rn << 5)
                | rd;
            prop_assert_eq!(w, want, "ARM ARM UBFM/UBFX field layout");
            prop_assert_eq!(w >> 31, sf, "sf");
            prop_assert_eq!((w >> 29) & 0b11, 0b10, "opc=10 UBFM");
            prop_assert_eq!((w >> 23) & 0x3f, 0b100110, "bits[28:23]");
            prop_assert_eq!((w >> 22) & 1, sf, "N=sf");
        ...
        #[test]
        fn encode_ubfx_metamorphic_rd_rn(
            is_64 in any::<bool>(),
            rd in 0u32..=30,
            rn in 0u32..=30,
            lsb_w in 0u32..=1,
        ...
        fn encode_ubfx_neg_arity(
            ...
            ops.truncate(len);
            prop_assert!(
                encode_ubfx(&ops).is_err(),
                "UBFX with {} operands must Err (llvm-mc: too few operands)",
                len
            );
        }

        #[test]
        fn encode_ubfx_neg_extra_operand(
            (is_64, rd, rn, lsb, width) in ubfx_valid(),
            extra in extra_operand(),
        ) {
            let mut ops = ops4(is_64, rd, rn, lsb as i64, width as i64).to_vec();
            ops.push(extra);
            prop_assert!(
                encode_ubfx(&ops).is_err(),
                "UBFX has no 5th operand; extra operand must Err (llvm-mc rejects it)"
            );
        }

        #[test]
        fn encode_ubfx_neg_sp(
            which in 0u32..=1,
            sp64 in any::<bool>(),
            is_64 in any::<bool>(),
            other in 0u32..=30,
            ...
            prop_assert!(
                encode_ubfx(&ops).is_err(),
                "SP/WSP is not a valid UBFX operand (which={} sp={})",
                which,
                sp
            );
        }
        ...
        fn encode_ubfx_neg_lsb_width(
            ...
            prop_assert!(
                must_err(&ops),
                "UBFX lsb={} width={} R={} must Err (ARM: 0<=lsb<R, 1<=width<=R-lsb)",
                lsb,
                width,
                r
            );
        ...
        #[test]
        fn encode_ubfx_diff_alt_spellings(
            (is_64, rd, rn, lsb, width) in ubfx_valid(),
            dest_spell in 0u32..=4,
            src_spell in 0u32..=4,
        ) {
            ...
            let mc = llvm_mc_word(&asm).expect("llvm-mc");
            let sut = sut_word(&ops).expect("SUT");
            prop_assert_eq!(sut, mc, "UBFX alt-spelling mismatch for {}", asm);
        }

        #[test]
        fn encode_ubfx_neg_mixed_width(
            rd in 0u32..=31,
            rn in 0u32..=31,
            rd64 in any::<bool>(),
            rn64 in any::<bool>(),
            ...
            prop_assert!(
                encode_ubfx(&ops).is_err(),
                "UBFX mixed W/X (rd64={} rn64={}) must Err (llvm-mc rejects it)",
                rd64,
                rn64
            );
        }
        ...
        fn encode_ubfx_neg_fp(
            ...
            prop_assert!(
                encode_ubfx(&ops).is_err(),
                "FP/SIMD register {} is not a valid UBFX operand (which={})",
                fp,
                which
            );
        }
        ...
        #[test]
        fn encode_ubfx_neg_invalid_name(
            which in 0u32..=1,
            name in invalid_name(),
        ) {
            let mut ops = ops4(false, 0, 1, 0, 1);
    ...
    #[test]
    fn test_encode_ubfx_regression_extra_operand() {
        let mut ops = ops4(false, 0, 0, 0, 1).to_vec();
        ops.push(Operand::Reg("x0".into()));
        assert!(
            encode_ubfx(&ops).is_err(),
            "UBFX w0, w0, #0, #1, x0 must Err; extra operand is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfx_regression_sp() {
        let ops = [
            Operand::Reg("wsp".into()),
            Operand::Reg("w0".into()),
            Operand::Imm(0),
        ...
        assert!(
            encode_ubfx(&ops).is_err(),
            "UBFX wsp, w0, #0, #1 must Err; register 31 is ZR not SP/WSP (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfx_regression_width_zero() {
        let ops = ops4(false, 0, 0, 0, 0);
        let result = catch_unwind(AssertUnwindSafe(|| encode_ubfx(&ops)));
        assert!(
            matches!(result, Ok(Err(_))),
            "UBFX w0, w0, #0, #0 must Err; width=0 is outside 1..=32-lsb (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfx_regression_mixed_width() {
        let ops = [
            Operand::Reg("x0".into()),
            Operand::Reg("w0".into()),
            Operand::Imm(0),
        ...
        assert!(
            encode_ubfx(&ops).is_err(),
            "UBFX x0, w0, #0, #1 must Err; mixed W/X is invalid (llvm-mc rejects it)"
        );
    }

    #[test]
    fn test_encode_ubfx_regression_fp() {
        let ops = [
            Operand::Reg("d0".into()),
            Operand::Reg("x1".into()),
            Operand::Imm(0),
        ...
        assert!(
            encode_ubfx(&ops).is_err(),
            "UBFX d0, x1, #0, #1 must Err; FP/SIMD registers are not UBFX operands (llvm-mc rejects it)"
        );
    }
}
