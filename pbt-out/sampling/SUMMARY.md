...
| One big category | **Instruction-encoding contract not enforced** — 506/506 real defects. C1–C8 are the eight small categories *derived from* this one big category (where in the compiler the missing check lives), not eight independent diseases. |
...
### 3.1 Classification (one big category, then eight derived small categories)

**Step 1 — one big category.** All **506 real defects** are this one functional-logic failure of ccc:

| Big category | Definition | Real defects |
|---|---|---|
| **Instruction-encoding contract not enforced** | ccc does not enforce the architectural / language contract that gcc/clang enforce. | **506** |

**Step 2 — eight small categories derived from that big category**, by *where in the compiler* the missing check lives (`src/` paths). C1–C8 are not independent diseases; they partition the 506. Counts are re-derived from the 509-row tracker by deterministic rules; the complete per-issue assignment is in `classification_per_issue.tsv`. CWE labels are dropped — see 3.2.

| Small category (derived) | Missing / wrong mechanism in ccc | ccc code location | Real defects | % of 509 |
|---|---|---|---|---|
...
| 一个大类 | **指令编码契约未强制执行** —— 506/506 条真实缺陷。C1–C8 是由该大类*导出*的八个小类（按缺失检查位于编译器何处划分），不是八种独立疾病。 |
...
### 3.1 分类（先一个大类，再导出八个小类）

**第一步 —— 一个大类。** 全部 **506 条真实缺陷**是 ccc 的这一种功能逻辑失效：

| 大类 | 定义 | 真实缺陷 |
|---|---|---|
| **指令编码契约未强制执行** | ccc 不强制执行 gcc/clang 所强制的架构/语言契约。 | **506** |

**第二步 —— 由该大类导出的八个小类**，按*缺失的检查位于编译器何处*（`src/` 路径）划分。C1–C8 不是八种独立疾病，而是对 506 条的划分。计数由确定性规则从 509 行 tracker 重新推导；逐条分配见 `classification_per_issue.tsv`。CWE 标签弃用——见 3.2。

| 小类（由大类导出） | ccc 中缺失/出错的机制 | ccc 代码位置 | 真实缺陷 | 占 509 |
|---|---|---|---|---|
